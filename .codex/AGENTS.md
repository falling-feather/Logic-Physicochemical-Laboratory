...
V6.6.63 后端阶段已经完成；V7.4.12—V7.4.30 完成项目对接、前端注册/生命周期、认证与角色资源、组织治理及目标发布仓库门禁；V7.4.31—V7.4.36 完成统一三星系总览、单一全局治理、角色落点、9001 一键入口和独立集中验收，R5c 已关闭；V7.4.37 又把三角色页面统一纠正为星序 `astra` 工作区并完成教师四分区重构。V7.5.x 已完成代码空间、未来星系、课程编排、教师学习进度闭环和全仓 review；V7.5.11 将 `main`、`qianduan`、`houduan`、`review` 统一到同一基线，后续开发回到 `houduan`。公网 R6 继续暂缓，当前任务与恢复条件只认 `doc/02-项目规划.md`。
...
1. 当前任务是“星序 Astra｜主开发｜总控”，主开发集中分配后续小版本并独占 `houduan` 根工作区写入；`main` 只接收已收束的集成基线。
2. 代码空间组、未来星系组、课程平台组和 QA 组已在本轮 review 后归档。后续任何小组没有在 `doc/02-项目规划.md` 登记正式任务、独立 worktree、允许路径和版本令牌时不得修改仓库。
...
5. `houduan` 项目对接提交使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`，责任组、任务编号与标题必须一致；`review` 当前冻结，只有用户重新启动 review 周期后才恢复 `reN：中文说明`。
...
- `houduan` 主线版本使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`，完成验收后再同步到 `main`。
