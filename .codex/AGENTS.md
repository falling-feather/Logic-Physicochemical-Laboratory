...
V6.6.63 后端阶段已经完成；V7.4.12—V7.4.30 完成项目对接、前端注册/生命周期、认证与角色资源、组织治理及目标发布仓库门禁；V7.4.31—V7.4.36 完成统一三星系总览、单一全局治理、角色落点、9001 一键入口和独立集中验收，R5c 已关闭；V7.4.37 又把三角色页面统一纠正为星序 `astra` 工作区并完成教师四分区重构。V7.5.x 当前用于代码空间、未来星系、课程编排与教师学习进度闭环；公网 R6 继续暂缓，当前任务与恢复条件只认 `doc/02-项目规划.md`。
...
1. 当前任务是“星序 Astra｜主开发｜总控”，主开发集中分配 `V7.5.x` 小版本并独占 `houduan` 根工作区写入。
2. 代码空间组、未来星系组和课程平台组只在 `doc/02-项目规划.md` 登记的独立 worktree 中写入；QA 复用长期只读任务。任何小组没有正式任务、允许路径和版本令牌时不得修改仓库。
3. QA 必须独立验收主开发或专业组交付；主开发自测不能替代 QA 回执。
4. `qianduan` worktree 含未提交既有成果，当前禁止新增写入、覆盖、搬移或清理；归属和状态以 `02` 登记为准。
5. `houduan` 项目对接提交使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`，责任组、任务编号与标题必须一致；`review` 分支按用户约定使用 `reN：中文说明`。
...
- `houduan` 主线版本使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`。
