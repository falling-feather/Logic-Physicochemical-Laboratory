...
V6.6.63 后端阶段已经完成；V7.4.12—V7.4.30 完成项目对接、前端注册/生命周期、认证与角色资源、组织治理及目标发布仓库门禁；V7.4.31—V7.4.36 完成统一三星系总览、单一全局治理、角色落点、9001 一键入口和独立集中验收，R5c 已关闭；V7.4.37 又把三角色页面统一纠正为星序 `astra` 工作区并完成教师四分区重构。公网 R6 按用户要求暂缓，当前任务与恢复条件只认 `doc/02-项目规划.md`。
...
2. FE、UI、OPS、QA 长期任务已在 V7.4.36 归档；未来若重新组建，仍默认只读，没有 `doc/02-项目规划.md` 中的正式任务、主开发写锁和版本令牌时不得修改仓库。
