...
V6.6.63 后端阶段已经完成；V7.4.12—V7.4.30 完成项目对接、前端注册/生命周期、认证与角色资源、组织治理及目标发布仓库门禁；V7.4.31—V7.4.36 完成统一三星系总览、单一全局治理、角色落点、9001 一键入口和独立集中验收，R5c 已关闭；V7.4.37 又把三角色页面统一纠正为星序 `astra` 工作区并完成教师四分区重构。V7.5.x 已完成代码空间、未来星系、课程编排、教师学习进度闭环和全仓 review；V7.5.13 在 `houduan` 建立 V8 终局规划，V7.6 起由 `主开发` 作为活动集成分支，前后端写入组使用隔离 worktree，`main` 保持稳定发布。V7.6.2 冻结“角色任务优先、三星系目录其次、浏览不等于完成”的产品契约；V7.6.4 已通过 QA-014 A01，13 项能力、跨层导入、全局 owner 与资源代际门禁重新冻结。V7.6.5 / BE-006 已形成权威学习证据与完成规则集成候选，只有 A02 独立验收通过后才冻结服务端合同；前端组继续 HOLD，不得提前接线。公网 R6 继续暂缓，任务与恢复条件只认 `doc/02-项目规划.md`。
...
3. `doc/02-项目规划.md` 与 `doc/03-开发历史.md` 由主开发集中写入；前后端组只维护各自专属实现文档差异并在 handoff 中提供任务/历史摘要，禁止并行修改共享 02/03 造成冲突。
4. QA 必须独立验收主开发或专业组交付；主开发自测不能替代 QA 回执。
5. `qianduan` worktree 已与 V7.5.11 基线对齐并保持干净；同步前的 V7.2 未提交实验完整保存在本地标签 `archive/qianduan-wip-before-main-sync-20260727`，该标签只用于恢复和审计，不是开发入口。
6. `主开发` 及其任务分支使用 `Vx.y.z GROUP type(scope): 中文说明（TASK-ID）`，责任组、任务编号与标题必须一致；`review` 当前冻结，只有用户重新启动 review 周期后才恢复 `reN：中文说明`。
