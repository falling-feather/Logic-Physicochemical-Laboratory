...
| GET | `/api/render/page/{slug}` | 前端可渲染页面结构；公开响应会移除原始脚本引用，仅保留带稳定 `sandboxId` 的 `scriptManifest`，并返回 `X-Astra-Content-Script-*` 沙箱契约头 |
