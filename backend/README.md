...
| GET | `/api/render/script-sandboxes/{sandbox_id}/page/{slug}` | 按已发布私有 schema 生成内容脚本 sandbox HTML；脚本标签只引用后端受控资产端点，响应带真实 `Content-Security-Policy`、`X-Astra-Content-Script-Iframe-Sandbox`、`nosniff` 与 `Referrer-Policy`，blocked/歧义/外部未镜像资产 fail closed |
