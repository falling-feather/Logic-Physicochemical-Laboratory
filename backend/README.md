...
| GET | `/api/render/script-sandboxes/{sandbox_id}/page/{slug}` | 按已发布私有 schema 生成内容脚本 sandbox HTML；每次响应生成运行时 CSP nonce，`script-src` 与 bootstrap `<script nonce>` 同步绑定，脚本标签只引用后端 bootstrap 端点，blocked/歧义/外部未镜像资产 fail closed |
...
| GET | `/api/render/script-sandboxes/{sandbox_id}/assets/{asset_sha256}/page/{slug}` | 按 published slug + sandboxId + 脚本引用 SHA-256 读取受控本地 JS 资产；仅允许 `pages/`、`shared/js/`、`codevis/shared/js/` 和 `drafts/` 根内 `.js` 文件，响应 `application/javascript`、asset hash 和 `nosniff` |
