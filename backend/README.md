...
| GET | `/api/render/script-sandboxes/{sandbox_id}/page/{slug}` | 按已发布私有 schema 生成内容脚本 sandbox HTML；脚本标签只引用后端 bootstrap 端点，响应带真实 `Content-Security-Policy`、`X-Astra-Content-Script-Iframe-Sandbox`、`nosniff` 与 `Referrer-Policy`，blocked/歧义/外部未镜像资产 fail closed |
| GET | `/api/render/script-sandboxes/{sandbox_id}/bootstrap/page/{slug}` | 返回 sandbox 内后端受控 bootstrap JS；按 published slug + sandboxId 生成受控资产 URL 列表，提供 `bootstrap-v1`、ready/error/postMessage 契约和 nonce 传递预留，不暴露原始脚本路径 |
