...
from app.services.access_control import require_class_teacher_or_admin_by_id, teacher_class_ids
from app.services.admin_common import (
    PENDING_SUBMISSION_STATUSES,
    change_snapshot,
    contains_pattern,
    next_offset as page_next_offset,
    require_admin,
    statement_count,
)
...
from app.services.external_issue_providers import build_issue_provider_adapter, external_issue_sync_posture
...
def list_pending_submissions(
    ...
    next_offset = page_next_offset(total, offset, len(items))
...
def list_bug_external_sync_operations(
    ...
    return BugExternalSyncOperationPage(
        ...
        limit=limit,
        offset=offset,
        next_offset=page_next_offset(total, offset, len(operations)),
    )
