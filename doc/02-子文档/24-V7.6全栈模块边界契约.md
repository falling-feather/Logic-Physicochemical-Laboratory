...
1. 遗留巨型文件不超过冻结行数。
2. 新 endpoint 不直接导入 ORM model；既有 endpoint/model import block 不静默扩大。
3. model/schema 不向 API 或 service 反向依赖；既有跨层例外不增加。
4. 学习证据 API 字面量只允许出现在共享 learning-evidence client。
5. `AstraRoleHomeClient`、`AstraLearningEvidenceClient`、`AstraLearningEvidenceQueue` 和 `AstraProductCapabilities` 各有唯一全局 owner。
6. `LearningProgress.markVisited` 不增加调用者。
7. 不新增资源版本常量表。
8. 产品能力注册表键、状态、证据来源和禁止声明完整且不可变。
9. 正向扫描器带有仓库内无副作用反例，能识别 endpoint→ORM、service→API、endpoint→peer endpoint、页面直连学习证据 API、重复全局 owner 和新增版本表。
