...
>
> **当前基线**：V7.4.37 已完成星序角色层级纠偏。`#student/#teacher/#admin` 统一登记为 `astra` 角色工作区，与工科试验室、代码空间、未来星系三个学习单元共同归属星序，不再继承工科 navbar、页脚或课程壳；教师端已拆为教学总览、组织与课程、作业发布、批改与学情四分区。V7.4.32—V7.4.36 的开放式三星系总览、单一全局治理、三角色星序落点、9001 同源入口、启动时预置管理员证明契约与集中验收继续有效；V7.4.21—V7.4.30 的认证、角色资源、组织治理、实验生命周期和 target-release-v2/51 项仓库门禁保持有效。用户已把域名与公网 R6 后置，本机结论不替代目标环境证据。
>
...
| GET/POST | `/api/learning-events` | 学习事件查询 / 记录访问、提交、完成等事件；学生读写除 published/active 规则外还要求当前班级分块为 open，读取排除 hidden 分块历史；教师查询按本班 teacher scope 收束 |
...
- `#student` 是星序内的学生学习工作台入口，属于 `astra` 身份工作区；在 `AstraPageRegistry` 中登记 `galaxy: 'astra'`、`roles: ['student']`，不得加入 `course` 标签，也不得复用课程页 `[data-module]` / `.content-section` 语义。
