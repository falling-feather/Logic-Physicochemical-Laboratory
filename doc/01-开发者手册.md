...
> **版本**: v6.3 规划约束同步（星系层级、UI 隔离与 support 生命周期规则）
> **文档大版本**: 6.x（覆盖 v5.0 / v5.1 / v6.x 多星系架构）
> **最后更新**: 2026-06-19 | **维护**: 星序 Dev Team
>
> 📝 **v6.0.0 项目层级调整与正式更名（2026-05-26）**：项目顶层品牌正式更名为「**星序 Astra**」。原"工科实验室"作为星序的一个**星系**保留为子站；同期独立化的「**代码空间**」（CodeSpace，目录代号 `codevis/`）作为星序的第二个星系正式上线。本文档面向"星序整体 + 双子站"重写头部与新增章节 17"新增星系开发指南"，其余 v4.x 技术细节章节保持稳定。完整文档体系：[02-更新规划.md](02-更新规划.md) · [03-发布历史.md](03-发布历史.md) · [04-部署指南.md](04-部署指南.md) · [05-UI规范模板.md](05-UI规范模板.md) · [99-历史审视报告归档.md](99-历史审视报告归档.md)。
>
...
│  /index.html      │      │  /codevis/         │           │  v6.3 待重建 UI     │
├──────────────────┤      ├──────────────────┤           ├───────────────────┤
│ 数学 · 物理 · 化学│      │ JS · Python · C/C++│           │ 地球与宇宙 · 材料    │
│ 算法 · 生物       │      │ 代码执行追踪播放器 │           │ 工程 · 数据 · 信息等 │
│ 88 个可视化实验   │      │ 独立 SPA + cv- 前缀│           │ 禁止照搬 englab UI  │
└──────────────────┘      └──────────────────┘           └───────────────────┘
      ▲                             ▲                               ▲
...
|---|---|---|---|
| **角色** | 核心数理工科知识星系 | 独立代码执行可视化星系 | 跨学科探索星系，v6.3 需要重建 UI |
| **一级入口** | 从星序总览进入 | 从星序总览外链进入 | 从星序总览进入 |
| **二级目录** | 数学 / 物理 / 化学 / 算法 / 生物 | 代码追踪及后续代码主题 | 地球与宇宙 / 工程 / 数据 / 信息 / 材料 / 人文 |
| **页面品牌** | 工科试验室 | 代码空间 | 未来星系 |
| **命名空间** | 现有主站命名；v6.3 后不得被未来星系继续复用 | `cv-` CSS 前缀 + `Cv*` 全局对象 | 需建立独立前缀，禁止使用 englab 页面壳 |
| **UI 外壳** | 自有顶栏、侧边栏、页脚 | 独立 SPA 外壳 | v6.3 重新设计，不能照搬工科试验室 |
| **Service Worker** | 主站 sw.js 缓存 | 由主站 sw.js 同样接管子站资源 | 主站 sw.js 缓存，但 UI 和资源命名需隔离 |
...
- **工科试验室**（englab；历史文档中曾写作“工科实验室”）：数学、物理、化学、算法、生物五大学科 88 个可视化交互实验，全面对齐人教版（2019 新课标）高中必修及选择性必修核心知识点。
- **代码空间**（codespace，目录代号 `codevis/`）：JavaScript / Python / C/C++ 三语言**手写代码沙箱** + 代码执行**逐步追踪播放器**，把变量变化、指针移动、数据结构变形可视化为时间轴动画。
- **未来星系**（frontier）：跨学科探索星系，当前已有 6 个 MVP 内容方向；v6.3 必须重建独立 UI 外壳，禁止继续沿用工科试验室页面布局。
...
- 🪐 **项目顶层品牌正式更名**：`工科实验室` → `星序 Astra`
  ...
  - 顶层架构定义：星序 = 工科实验室星系（englab） + 代码空间星系（codespace）+ 未来星系
...
- 📖 **开发者手册全方位重写**：
  - 新增 §0 "总览：星序的双子站架构"
  - §1/§2 重写为面向星序整体的视角（含双子站对照表 + 双子站架构图）
