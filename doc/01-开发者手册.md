...
V6.6.47 起，`?backendSchema=1` 的前端试点不再只渲染学习任务卡；adapter 会读取 render API 中的 `scriptManifest.embed` 并创建 iframe sandbox 容器。该能力仍是 opt-in 接入层，不改变默认静态页面回退策略。
