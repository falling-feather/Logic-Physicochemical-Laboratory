...
> **版本**: V6.6.54 后端收束；Python 业务后端与 C++ 静态服务处于并存过渡期
...
>
> ✅ **当前进度状态（2026-07-10）**：`houduan` 分支已形成 FastAPI 业务后端、连续迁移、内容/认证/教学/治理 API、管理/教师/学生三端入口、内容脚本隔离执行链和生产试运行门禁。V6.6.54 已在本地 outbox/dispatch plan 上补齐默认关闭的 HTTPS Webhook 显式投递、dispatching/delivered/failed 状态、脱敏信封/审计和人工重试；既有静态前端路径继续保持可回滚。完整历史能力与验证证据见 `03-发布历史.md`。
...
| `app.services.alert_delivery` | V6.6.54 外部告警适配器；默认关闭，只接受安全配置注入的 HTTPS Webhook URL 和 SecretStr token。显式 plan dispatch 采用 Bearer + HMAC-SHA256 + 稳定 Idempotency-Key，只发送 `astra.alert-envelope.v1` 脱敏信封；条目先提交 dispatching，再逐项落 delivered/failed 与审计。网络/5xx/429 失败可人工重新排队，同 payload 重试沿用幂等键；不自动扫描、不接 broker，进程中断恢复归 V6.6.55 |
...
- 学校、班级、课程范围判断优先复用 `app.services.access_control`。课程 owner 转让仍只允许 current owner 或全局 admin；课程协作者支持 `editor/content_editor/assessment_editor/viewer` 与 owner/admin 批量 upsert，分别约束结构、作业/评估、积分规则和只读能力，但任何课程角色都不会自动授予班级成员、挂课、提交、评分或学情权限。作业通过 `audience_mode` 与 `AssignmentClassPolicy` 按班级覆盖 assigned、状态、截止时间和积分规则；班级策略写入同时要求课程编辑能力与目标班级 active teacher scope。剩余外部治理、真实 MySQL、性能/安全与发布候选门禁按 `02-更新规划.md` 和 `09-后端阶段收束小版本开发安排.md` 推进。
