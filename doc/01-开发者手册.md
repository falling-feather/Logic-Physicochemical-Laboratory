...
>
> **当前基线**：V7.4.5 管理 API alert outbox 拆分 + V7.4.4 知识快照任务拆分 + V7.4.3 内容治理领域拆分 + V7.4.2 组织领域拆分 + V7.4.1 用户/权限领域拆分 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
...
| `app.api.endpoints.admin` / `admin_users` / `admin_organizations` / `admin_content` / `admin_snapshot_tasks` / `admin_alerts` | `admin.py` 保持 `/api/admin` 聚合与尚未拆分领域的兼容入口；子模块承载用户、组织、内容、知识快照和告警控制面。新增拆分必须保持路由顺序、路径、响应 schema、状态码和审计 action 不变 |
