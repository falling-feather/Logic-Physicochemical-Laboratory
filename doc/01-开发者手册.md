...
>
> ✅ **当前进度状态（2026-07-03）**：`houduan` 分支已新增 `backend/` FastAPI 后端骨架、数据库探针、Alembic 迁移、`/api/health`、部署预检、API no-store 缓存边界、本地账号认证安全基线、学校班级最小闭环、课程/作业/学习事件/提交批改/积分统计、知识状态/班级规则统计、个人/班级知识快照、管理端 API、管理端列表分页搜索、审计元数据与认证事件审计、`physics/energy-conservation` 内容协议持久化样例和前端 opt-in schema 渲染试点；既有前端路径保持可回滚。
...
│   │   ├── models/              # 用户、会话、学校、班级、课程、作业、学习事件、内容页 ORM 模型
...
FastAPI 后端位于 `backend/`，当前首切片提供配置、数据库探针、健康检查、部署预检、API no-store 缓存边界、内容协议样例、本地认证安全基线、课程/作业/学习事件、提交批改、积分流水、个人进度摘要、知识状态/班级规则统计、个人/班级知识快照、管理端基础 API、管理端列表分页搜索、审计元数据与认证事件审计、前端 opt-in schema 试点和测试入口。
