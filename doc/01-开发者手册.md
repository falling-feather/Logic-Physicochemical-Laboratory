...
| `app.services.admin_alert_outbox` | 管理端告警 outbox 人工复核台账；首轮把知识快照告警候选显式写入 `admin_alert_outbox_entries`，按 source/run/code/action dedupe 幂等刷新 `seen_count/last_seen_at`，payload 仅保留脱敏运行摘要；`external_delivery=false`、`dispatch_mode=manual_review`，不发送邮件/短信/Webhook、不自动处置 |
