...
| 服务 | 职责 |
| ---- | ---- |
| `app.services.access_control` | 学校、班级、课程可见性与角色范围判断；普通业务端点应复用它，避免 `_require_*` 逻辑漂移；教师端班级挂课、学习事件、积分流水、学生进度、作业提交列表与批改必须落到 active class teacher scope，全局 admin 保留跨范围治理能力 |
