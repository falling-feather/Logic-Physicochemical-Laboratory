...
>
> **当前基线**：V7.5.5 已完成代码空间和未来星系课程化重绘：两者均使用 6 组课程目录、18 个独立子课和可完成互动；未来星系另以 6 个按需 Canvas owner 和单一 Three.js 代表课解释跨学科问题。V7.5.3 的三星系通用发布计划和 V7.4.37 的星序角色层级纠偏继续有效。`#student/#teacher/#admin` 仍统一登记为 `astra` 角色工作区，与工科试验室、代码空间、未来星系三个学习单元共同归属星序；浏览器反馈不是权威判题，正式提交等待 BE-005。用户已把域名与公网 R6 后置，本机结论不替代目标环境证据。
>
...
- **未来星系**（frontier）：跨学科探索星系，当前包含地球与宇宙、工程应用、数据科学与 AI、信息技术、材料与微观结构、语言与人文可视化 6 个课程群、18 个稳定活动；总览只负责发现，六方向页面负责单目标子课，Canvas/Three 互动和页面级运行态清理均已接入。
...
### 6.6 代码空间子站（codespace 星系，V7.5.4）
...
| 可执行挑战 | `codevis/pages/course-challenge/` | 预测、运行、追踪、修正、公开预检与禁用的正式提交入口 |
...
- `CvCourseStateAdapter` 的权威 HTTP 模式必须提供明确 `course_ids + class_id`；返回缺失活动按 hidden，响应失败按 unavailable。学生上下文的自动注入由后续共享学习上下文接入完成，不得用前端默认值替代服务端授权。
- 第三方运行时固定在 `codevis/vendor/`，许可证和哈希清单必须随资产更新；禁止改成分支 URL 或 CDN。学生页面不得显示运行时实现说明。

### 6.7 未来星系课程（frontier 星系，V7.5.5）

未来星系继续位于主站 SPA，但 V7.5.5 已把 `index.html` 中的六方向长页收缩为 7 个最小运行挂载点；课程 DOM 只由当前路由在进入时创建。
...
**关键约束**：
- `#frontier` 只显示课程目录；六方向 hash 使用稳定 `route_slug`，`activity_key` 只作前后端课程身份，不直接泄漏到 hash 或替代授权。
- `FrontierCourseManifest.configureHttp()` 必须收到明确的课程 ID 映射、`class_id` 和 same-origin fetch；BE-004 缺失活动按 hidden，任一响应失败则整份目录 unavailable。
