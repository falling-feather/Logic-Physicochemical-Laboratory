...
> **版本**: V6.6.51 后端收束；Python 业务后端与 C++ 静态服务处于并存过渡期
> **文档大版本**: 6.x（覆盖 v5.0 / v5.1 / v6.x 多星系架构）
> **最后更新**: 2026-07-10 | **维护**: 星序 Dev Team
>
...
>
> ✅ **当前进度状态（2026-07-09）**：`houduan` 分支已新增 `backend/` FastAPI 后端骨架、数据库探针、Alembic 迁移、`/api/health`、部署预检、部署 smoke、反向代理/服务注册拓扑报告脚本、API no-store 缓存边界、本地账号认证安全基线、活动会话列表与单会话撤销、会话设备标识与 last_seen 追踪/节流、管理员密码重置、用户自助密码重置令牌、密码重置 token 留存清理脚本、禁用用户会话撤销、用户名大小写规范化与 normalized key 数据库唯一约束、必填文本修剪后校验、学校班级最小闭环与加入申请审批、teacher direct join 审批收口、课程/作业/学习事件/提交批改/学生侧只读复盘/跨班级提交唯一性/学生资源状态可见性/积分统计、知识状态/班级规则统计、个人/班级知识快照、知识快照周期重算脚本/运行记录/进程内调度器/数据库租约防重入/自动心跳、管理端知识快照运行列表/健康摘要/调度积压摘要/告警候选摘要/告警 outbox 人工复核台账、状态流转、队列摘要与批量复核、协作式取消与手动 requeue、管理端 API、管理端加入申请队列、管理端列表分页搜索、管理端内容页数据库侧分页、缺陷记录外部 issue 链接、审计元数据与认证事件审计、审计日志链式哈希、审计链完整性校验、审计日志 JSON/CSV 明细导出、报表摘要导出、审计高频候选摘要、审计留存预检、本地审计归档包导出/Manifest 校验与导出/摘要行为审计留痕、`physics/energy-conservation` 内容协议持久化样例、内容 seed 启动初始化与正式内容初始化入口、ContentDraft 草稿/脚本审核/脚本静态分析/脚本 sandbox 契约/脚本资产 allowlist/SRI 静态门禁/脚本资产下载校验证据/发布版本绑定的外部脚本资产镜像/管理端脚本资产供应链清单/内容脚本镜像一致性审计/内容脚本远端漂移扫描/内容脚本扫描 run 台账与远端漂移告警候选摘要/内容脚本远端漂移扫描调度与 CLI observe-only 首轮/内容脚本扫描 run 健康与队列摘要/内容脚本远端漂移告警 outbox 人工复核入队与状态流转/内容脚本 CDN host 信任治理首轮/公开 render 脚本 manifest 脱敏与沙箱执行契约头/稳定 `sectionId/sourceId` 内容身份/草稿编辑/提交退回撤回工作流、active 草稿数据库唯一约束、内容发布/版本记录/追加式回滚、发布/回滚冲突 409、脚本历史版本 rollback 重审门禁、内容页 current 指针、草稿 base version/hash、版本 previous 链、发布元数据、中文路径/中文 slug 回归、管理端版本 JSON path diff 敏感预览脱敏与带显式稳定 ID 字段的 semantic 富语义摘要、前端 opt-in schema 渲染试点、前端 iframe 编排首轮、浏览器隔离证明、管理端治理 UI 只读总览首轮和教师端工作台 MVP 首轮；既有前端路径保持可回滚。

> ✅ **V6.6.47 增量（2026-07-09）**：前端 opt-in 后端 schema adapter 已消费 `scriptManifest.embed`，完成 iframe sandbox 状态卡、`apiBase` 相对地址解析、postMessage 校验、refresh/timeout/error 状态和模块切换清理首轮；浏览器隔离证明、旧实验脚本 sandbox DOM 模板化和稳定错误码继续进入 V6.6.48。

> ✅ **V6.6.48 增量（2026-07-09）**：新增 `tools/browser/script-sandbox-isolation-proof.cjs` 本地浏览器证明脚本，已用 Edge/Playwright 验证 iframe opaque origin 无法读取父页面 DOM、localStorage、sessionStorage、cookie 或 top location；sandbox HTML 的 `frame-ancestors` 只吸收 `ASTRA_CORS_ORIGINS` 中合法 origin，bootstrap/asset 可执行资源使用 `Cross-Origin-Resource-Policy: cross-origin` 以支持 opaque iframe 加载。

> ✅ **V6.6.49 增量（2026-07-09）**：主站新增 `#admin` 管理端治理入口，按现有 SPA 路由懒加载 `pages/admin/admin.js` / `pages/admin/admin.css`；首轮只读接入管理员门禁、全站统计、脚本/知识快照/outbox 健康摘要，以及加入申请、内容草稿、脚本资产、脚本 Host 策略、知识快照运行、告警 outbox、审计日志和 Bug 台账分页列表。状态变更、审批、复核、cancel/requeue、dispatch plan 和审计报表导出 UI 继续后续版本推进。

> ✅ **V6.6.50 增量（2026-07-09）**：主站新增 `#teacher` 教师端工作台入口，按 englab 非课程页懒加载 `pages/teacher/teacher.js` / `pages/teacher/teacher.css`；首轮接入教师/admin 门禁、学校/班级/课程范围选择、课程与作业创建、挂班、成员状态、协作者、待批改队列、评分、assignment 积分规则、单个学生进度和班级知识聚合摘要。课程/单元/作业/班级 PATCH/关闭/归档接口、批量进度、提交列表学生姓名、复杂成员操作和登录态端到端浏览器回归继续后续版本推进。

> ✅ **V6.6.51 增量（2026-07-10）**：主站新增 `#student` 学生端工作台入口，按 englab 非课程页懒加载 `pages/student/student.js` / `pages/student/student.css`；后端新增 `GET /api/classes?mine=true` 与分页 `GET /api/assignments/me`，作业中心按 active membership 展开班级、课程、单元、作业、提交和复盘状态，支持 `all/active/feedback/history` 过滤。学生无班级时只按教师提供的 `class_id` 直接加入；多班级复盘必须显式携带 `class_id`，重复提交统一返回 `409`，`due_at` 当前只作展示。完整离线/缓存收口进入 V6.6.52，细粒度统计与复杂权限进入 V6.6.53。
...
│ - 历史 REST API (/api/health, /api/info) │ │ - /api/render/page/{slug}│
└────────────────────────────────────────┘ │ - Admin / Auth / School  │
...
│   ├── 09-后端阶段收束小版本开发安排.md  # V6.6.37-V6.6.60 后端收束路线图
...
│   │
│   ├── admin/                  # 管理端治理 UI（V6.6.49 首轮，只读总览）
│   │   ├── admin.css           # 管理端统计、队列、表格和状态样式
│   │   └── admin.js            # 管理端门禁、API base、统计和治理队列读取
│   ├── teacher/                # 教师端工作台 UI（V6.6.50 首轮）
│   │   ├── teacher.css         # 教师工作台、课程/成员/批改/知识摘要样式
│   │   └── teacher.js          # 教师门禁、API base、范围选择和教学工作流读取/写入
...
| `app.services.content_catalog` | 内容页 seed、正式内容初始化、published schema 读取和内容页摘要；GET 查询只读当前已发布记录并剥离原始脚本引用、SRI/crossorigin 元数据和 sandbox 原始字段，只保留带稳定 `sandboxId` 的 `scriptManifest`，manifest 内含沙箱 enforcement/capabilities；`/api/render/page/{slug}` 还会返回 `X-Astra-Content-Script-*` 沙箱契约头，并对可执行且全页唯一的 manifest 注入 `scriptManifest.embed` 描述符。`embed` 只包含前端 iframe 编排所需的 `iframe.src/sandbox/referrerPolicy`、messageProtocol、capabilities、assetCount 和 opaque origin 语义，不包含 nonce、原始 URL/路径值、SRI、crossorigin、bootstrap URL、asset URL、镜像资产 metadata 或数据库 id；blocked/ambiguous manifest 不生成可执行 embed。前端接入时应消费 descriptor，不自行拼装 sandbox HTML/asset endpoint，也不把 descriptor 当作授权凭据；前端 opt-in adapter 已接入 iframe 生命周期首轮，负责 src 解析、状态机、ready/error/timeout 与模块切换清理；浏览器隔离证明已由 V6.6.48 本地脚本覆盖，旧脚本 sandbox DOM 模板化仍由后续版本完成。`/api/render/script-sandboxes/{sandbox_id}/page/{slug}` 从已发布私有 schema 生成 sandbox HTML，运行时 CSP nonce 只存在于单次 HTML 响应，脚本标签只引用后端 bootstrap 端点；sandbox HTML 的 `frame-ancestors` 保留 `'self'` 并只追加 `ASTRA_CORS_ORIGINS` 中合法 `http/https` origin，拒绝 `*` 或 `null` 进入可嵌入来源；`/api/render/script-sandboxes/{sandbox_id}/bootstrap/page/{slug}` 会生成 bootstrap JS、受控资产 URL 列表和 ready/error/postMessage 契约，并把 HTML nonce 传给动态资产脚本；bootstrap 与 asset 响应使用 `Cross-Origin-Resource-Policy: cross-origin`，以便 opaque sandbox iframe 加载受控可执行资源；`/api/render/script-sandboxes/{sandbox_id}/assets/{asset_sha256}/page/{slug}` 会重新绑定 published slug、当前 page version、sandboxId 和脚本引用 hash，本地脚本只读受控根，外部脚本只读发布事务写入的镜像资产，不在 GET 路径写库或联网；seed 在开发/测试初始化或显式初始化流程执行；内置 seed `2026.07-v6.5-schema.2` 已带稳定 `sectionId/sourceId`；正式初始化会写入 `content_pages` 与 `content_page_versions`，要求 active admin 发布归因，默认不覆盖已有差异版本；回归覆盖中文目录/中文 SQLite 文件名下的迁移与初始化 |
...
该命令检查数据库连通和 Alembic 当前 revision 是否到 head；生产环境追加 `--require-mysql` 时，还会要求连接为 MySQL、`ASTRA_AUTO_CREATE_TABLES=false`，并校验数据库/连接字符集为 `utf8mb4`、排序规则为 `utf8mb4_` 前缀。报告中的 `configuration` 会记录运行环境和自动建表状态，`compatibility` 会记录 MySQL 字符集、排序规则、时区、版本、`sql_mode`、`max_connections`、当前库和当前用户；其中时区、`sql_mode` 与连接数当前只报告不强制。部署前应先执行 `python -m alembic upgrade head`，再执行预检；本地 SQLite 或 CI 临时库只做快速验证时可不传 `--require-mysql`。真实 MySQL 演练证据写入发布历史，长期规划和未闭环项继续维护在 `02-更新规划.md` 与 `09-后端阶段收束小版本开发安排.md`。

**反向代理/服务注册拓扑报告**：

```bash
python -m scripts.deploy_topology_drill --static-url https://your-domain.example/ --proxied-api-url https://your-domain.example/api/health --direct-api-url http://127.0.0.1:8000/api/health --origin https://your-domain.example --api-bind-host 127.0.0.1
```

该命令检查静态主站、经反向代理的 FastAPI health、no-store、request id、CORS、直连 API 监听地址、公网直连端口暴露和 `EngLab` / `AstraApi` 服务守护计划。它不替代真实反向代理配置、Windows 服务注册、安全组和重启恢复留证；完整参数和证据口径见 `04-部署指南.md`。
...
该命令默认只读，用于初始化、发布、回滚演练前后检查 current 指针、schema hash、版本链、active 草稿、脚本镜像、API no-store 和可选静态 fallback。它不执行发布/回滚、不联网下载脚本、不写库，也不返回完整 schema、原始 CDN URL、完整 SRI、镜像字节或 `content_bytes`。真实 MySQL 并发结果和 `init_content_pages` apply 报告仍需按 `09-后端阶段收束小版本开发安排.md` 留证。
...
该脚本按日或自然周对齐时间窗，先抢占 `knowledge_snapshot_runs` 数据库租约，再重算活跃班级已挂接课程的个人/班级知识快照，并写入运行记录。进程内调度器通过 FastAPI lifespan 注册，默认关闭；启用后会按日/周窗口检查运行记录，成功窗口跳过，失败窗口按 `attempt_count` 与 `ASTRA_KNOWLEDGE_SNAPSHOT_RETRY_ATTEMPTS` 控制重试，并按 `ASTRA_KNOWLEDGE_SNAPSHOT_SCHEDULER_PENDING_LIMIT` 限量扫描 pending run 以消费管理员手动 requeue 的窗口。调度器和 CLI 使用同一租约路径，未过期 running 窗口会跳过，过期租约可被新 worker 接管；开始重算、完成成功和写入失败状态前都会用 `id/status/owner/token`、未过期 lease 和 heartbeat 字段确认租约仍有效，长重算循环会按 `ASTRA_KNOWLEDGE_SNAPSHOT_SCHEDULER_HEARTBEAT_SECONDS` 自动续租，已过期或字段不完整的租约不能续租或完成。CLI 非法参数/日期会返回 JSON 错误，成功报告不回显原始 metadata；管理员可分页查看运行记录，响应只返回 `metadata_summary` / `metadata_redacted`，不返回原始 `metadata_json` 或重排原因正文；健康摘要可观察 stale running、partial lease、即将过期 lease、claimable、retryable/exhausted failed 和 problem runs，通过调度积压摘要区分 dispatchable now、claimable by lease rule、manual requeue 和 blocked runs，通过告警候选摘要获取只读 severity/action hint，通过 outbox 入队把候选写入本地人工复核台账，协作式取消 pending 或带 scheduler lease 的 running run，并将 failed、cancelled 或过期带租约 running run 手动 requeue 为 pending。告警候选摘要和 outbox 都不投递外部消息、不自动处置 run；外部任务队列、正式告警投递、队列执行治理和真实 MySQL 长任务取消/重排演练仍需后续设计。
...
`audit_archive_drill` 默认只读，用于归档演练前后检查留存 cutoff、候选桶、归档预览、候选链完整性、敏感字段扫描和操作边界；它不写文件、不写审计、不删除或移动 `audit_logs`，报告不返回密码、token、密钥、原始快照正文或复核备注。`archive_audit_logs` 按 `created_at <= cutoff` 选择审计候选，支持 `--retention-days`、`--before`、常用审计筛选项、`--from/--to`、`--limit`、`--dry-run`、JSONL/CSV 输出和 Manifest 校验。Manifest 记录策略、筛选、导出数量、截断状态、首尾候选、链边界、hash-chain 重算状态、归档文件 SHA-256 和字节数；`--verify` 会复验 SHA-256、记录数和归档文件内部相邻 hash 链，JSONL 且显式 `--include-snapshot` 时可重算 `current_hash`，未导出快照或 CSV 只声明 partial 复核能力。脚本默认不删除源表、不写新的 `admin.audit.*` 审计事件、不提供 WORM 或外部锚定。
...
`backend_stage_gate` 默认只读，用于生成 V6.6.44 阶段完成判定 JSON。它会聚合各类既有 drill/preflight/smoke 报告，并输出 `gates`、`blockers`、`missing_evidence`、`warnings` 和“通过 / 延期 / 带风险通过”建议。`--confirm-*` 参数只表示操作者已经另行完成并留存对应证据；脚本不会自动运行完整 pytest、浏览器 storage 检查、核心人工路径或真实回滚演练。未使用 `--require-mysql`、未运行真实拓扑、或缺少确认项时，报告应保持 `missing_evidence` / “延期”。
...
| GET | `/api/render/script-sandboxes/{sandbox_id}/page/{slug}` | 返回内容脚本 sandbox HTML；按已发布私有 schema 定位 manifest，每次响应生成唯一 nonce，CSP `script-src` 与 bootstrap `<script nonce>` 同步绑定，`frame-ancestors` 只允许 `'self'` 和 `ASTRA_CORS_ORIGINS` 中合法 origin，missing/blocked/歧义/缺失镜像资产 fail closed |
| GET | `/api/render/script-sandboxes/{sandbox_id}/bootstrap/page/{slug}` | 返回 sandbox 内 bootstrap JS；按 published slug + sandboxId 绑定，生成受控资产 URL 列表、ready/error/postMessage 契约，并通过 `document.currentScript.nonce` 继承 HTML nonce 给动态资产脚本，响应 `application/javascript`、bootstrap version、asset count、`nosniff`、`no-store`、`no-referrer` 与 `Cross-Origin-Resource-Policy: cross-origin` |
...
- V6.6.47 起 adapter 会消费 `scriptManifest.embed` 创建 iframe sandbox 状态卡；相对 `embed.iframe.src` 按当前 API base 解析，父页只接受匹配 `source/sandboxId/bootstrap-v1` 的 postMessage。
- 状态卡会区分 loading、bootstrapping、assets、ready、error、timeout、blocked；旧实验脚本尚未完成 sandbox root 模板化时，可能只完成 bootstrap/assets 而不进入完整 ready。
- V6.6.48 起可从仓库根目录运行浏览器隔离证明：`node tools/browser/script-sandbox-isolation-proof.cjs --api http://127.0.0.1:8000 --web http://127.0.0.1:8766 --channel msedge --out test-screenshots\browser-isolation`；脚本会留下 JSON 报告和页面/iframe 截图。
...
- `#admin` 是主站内的管理端治理入口，属于 englab 壳内的非课程页；在 `shared/js/router.js` 中只配置 `pageScripts.admin`，不得加入 `coursePages`，避免触发 `ModuleSelector` 和实验工具链。
- `englab` 通用支持脚本必须加载本地 `shared/js/lucide.min.js`，保证直接进入 `#admin` 时图标按钮不会出现空占位；未通过 admin 门禁时 dashboard 必须保持 `hidden`，只展示受控门禁提示。
- `pages/admin/admin.js` 默认同源请求 `/api/*`，开发时可用 `?apiBase=http://127.0.0.1:8000` 或 localStorage `astra-admin-api-base` 指向独立 FastAPI；鉴权兼容 HttpOnly cookie 与本地调试 Bearer token。
- 首轮只读读取 `/api/users/me`、`/api/admin/stats`、脚本/知识快照/outbox health/queue 和主要治理列表；所有列表必须传 `limit/offset`，UI 不应一次性拉取无界数据。
- 状态变更类 API（审批、脚本 Host policy 更新、outbox 复核、dispatch plan、快照 cancel/requeue、Bug 更新等）在确认弹层、幂等失败态和操作后审计验证完成前，不应直接做成可执行按钮。

**教师端工作台首轮**：

- `#teacher` 是主站内的教师端工作台入口，属于 englab 壳内的非课程页；在 `shared/js/router.js` 中只配置 `pageScripts.teacher`，不得加入 `coursePages` 或复用课程页 `[data-module]` / `.content-section` 语义，避免触发 `ModuleSelector` 和实验工具链。
- `pages/teacher/teacher.js` 默认同源请求 `/api/*`，开发时可用 `?apiBase=http://127.0.0.1:8000`、`CONFIG.backend.apiBaseUrl` 或 localStorage `astra-teacher-api-base` 指向独立 FastAPI；鉴权兼容 HttpOnly cookie 与本地调试 Bearer token。
- 教师页先读取 `/api/users/me`，仅 `teacher/admin` 展示工作台；学生、未登录或 API 不可用时只展示门禁卡，dashboard 必须保持 `hidden`。
- 首轮页面只消费现有后端契约：`/api/schools`、`/api/classes`、`/api/courses`、课程协作者、课程单元/作业、`/api/admin/submissions/pending` teacher scope、`/api/submissions/{id}/grade`、`/api/points/assignments/{id}/rule`、`/api/progress/users/{id}` 和 `/api/classes/{id}/knowledge`。
- 不应在前端伪造尚未存在的后端能力。课程/单元/作业/班级 PATCH、关闭/归档、批量进度、提交列表学生姓名、移除/转班、批量导入、多角色/批量协作者和班级专属积分覆盖规则继续按 `02` / `09` 后续版本推进。

**学生端学习闭环首轮**：

- `#student` 是主站内的学生端工作台入口，属于 englab 壳内的非课程页；在 `shared/js/router.js` 中只配置 `pageScripts.student`，不得加入 `coursePages`，也不得复用课程页 `[data-module]` / `.content-section` 语义。
...
- 班级范围以 `GET /api/classes?mine=true` 的 active membership 为准。没有班级时不展示学校/班级公共目录，只允许学生输入教师提供的 `class_id` 调用 direct join；入班后重新读取本人班级和作业中心。“教师提供”当前只是 UI/运营约定，后端不验证 ID 来源，不能当作邀请码安全边界；正式邀请模型与可猜 ID 风险归入 V6.6.53。
...
- 学生概览可消费个人 progress、points、knowledge 与 snapshots，并基于现有规则统计生成确定性的补强建议；不得伪装为 AI 推断，也不得在首轮自动触发快照重算写入。完整失败态、离线与缓存硬化进入 V6.6.52，细粒度统计和复杂权限进入 V6.6.53。
...
当前生产部署处于双服务过渡期：C++ 服务器继续承载静态站和历史 fallback，Python FastAPI 承载 `/api/*` 业务接口。推荐在云服务器上以反向代理作为公网入口，`/*` 分流到 C++ 静态服务 `:910`，`/api/*` 分流到 FastAPI `:8000`；FastAPI 应只监听 `127.0.0.1` 或内网地址，不直接开放公网端口。
...
.\deploy.ps1
# C++ 静态服务将在端口 910 启动
# 启用 Python API 时按 04-部署指南注册 AstraApi 并配置反向代理
```
...
| V6.6.37 至 V6.6.60 后端阶段收束小版本安排 | `09-后端阶段收束小版本开发安排.md` |
