...
- 学校、班级、课程范围判断优先复用 `app.services.access_control`；剩余管理端列表/队列权限矩阵、MySQL 联调和前端 schema 自动回归按 `02-更新规划.md` 推进。
