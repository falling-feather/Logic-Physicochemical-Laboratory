...
- 学校、班级、课程范围判断优先复用 `app.services.access_control`；课程协作者通过 `CourseCollaborator`、`GET/POST /api/courses/{id}/collaborators` 与 `PATCH /api/courses/{id}/collaborators/{collaborator_id}` 收束，课程创建者或全局 admin 可管理 active editor，active editor 可创建单元/作业和维护 assignment 级积分规则，但不能管理协作者；assignment 级积分规则通过 `app.services.points` 和 `/api/points/assignments/{id}/rule` 收束，普通班级教师可按规则批改自己班级提交，但非课程创建者/非 active editor 不能改写共享作业规则；剩余管理端列表/队列权限矩阵、MySQL 联调和前端 schema 自动回归按 `02-更新规划.md` 推进。
