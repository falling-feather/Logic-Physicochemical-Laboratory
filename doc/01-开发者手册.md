...
>
> **当前基线**：V7.4.25 已完成管理组织权威状态可视化、受限编辑/归档对话框、歧义写入对账锁和归档教学只读边界 + V7.4.24 已完成学校/班级受约束治理、Alembic 0047 与活动组织写入/当前统计边界 + V7.4.23 三角色 CSS/JS 资源级裁剪已通过 QA-008 独立验收 + V7.4.22 浏览器 Cookie-only/非浏览器 Bearer 双通道 fail-closed 契约 + V7.4.21 已恢复 09 号现行后端阶段追踪 + V7.4.20 已修复工科首页 390px 根级横向溢出 + V7.4.19 已修正 DNA/遗传模式 wrapper 真实 DOM 挂载 + V7.4.18 已验证 23 项实验整页离开 cleanup、补齐 2 项缺失取消协议 + V7.4.11 Python/Node/C++ 依赖与产物追踪完成 + V7.4.8 管理 API 全部分域拆分完成 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
...
- V7.4.25 起，上述证明还覆盖管理组织 active/archived 统计、白名单元数据修改、归档/恢复、2xx 权威不一致、409、发送后歧义、显式对账和 PATCH 进行中离开/重进 admin 的生命周期竞态；旧 lifecycle 完成后不得清除新页面 busy 或写锁，显式对账不得重发 PATCH。该脚本仍只证明一次性 SQLite 与本机 Edge，不替代 QA-007 或目标环境发布证据。
...
- 组织治理使用原生 dialog：busy 时 Escape 不关闭且焦点落到标题，空闲时 Escape 可关闭；成功、锁定和解锁分别移动到状态区或可操作入口，触发器被重建时重新定位。编辑、刷新和对话框操作目标至少 44×44，390×844 不产生根级水平溢出。当前角色静态资源、HTTP fallback、Service Worker 和缓存名称统一使用 `20260716v7425OrganizationGovernanceP0`。
