...
>
> **当前基线**：V7.4.3 管理 API 内容治理领域拆分 + V7.4.2 组织领域拆分 + V7.4.1 用户/权限领域拆分 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
...
| `app.services.admin_common` | 管理领域共享管理员角色校验、数据库计数、分页游标、转义搜索模式、审计变更快照和待批改提交状态口径；端点模块不得复制这些规则 |
...
| `app.api.endpoints.admin` / `admin_users` / `admin_organizations` / `admin_content` | `admin.py` 保持 `/api/admin` 聚合与尚未拆分领域的兼容入口；子模块分别承载用户/权限、组织，以及内容页/草稿/版本/脚本资产治理。新增拆分必须保持路由顺序、路径、响应 schema、状态码和审计 action 不变 |
