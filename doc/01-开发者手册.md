...
| `app.services.content_catalog` | 内容页 seed、正式内容初始化、published schema 读取和内容页摘要；GET 查询只读当前已发布记录并剥离原始脚本引用、SRI/crossorigin 元数据和 sandbox 原始字段，只保留带稳定 `sandboxId` 的 `scriptManifest`，manifest 内含沙箱 enforcement/capabilities；`/api/render/page/{slug}` 还会返回 `X-Astra-Content-Script-*` 沙箱契约头；`/api/render/script-sandboxes/{sandbox_id}/page/{slug}` 从已发布私有 schema 生成 sandbox HTML，运行时 CSP nonce 只存在于单次 HTML 响应，脚本标签只引用后端 bootstrap 端点；`/api/render/script-sandboxes/{sandbox_id}/bootstrap/page/{slug}` 会生成 bootstrap JS、受控资产 URL 列表和 ready/error/postMessage 契约，并把 HTML nonce 传给动态资产脚本；`/api/render/script-sandboxes/{sandbox_id}/assets/{asset_sha256}/page/{slug}` 会重新绑定 published slug、sandboxId 和脚本引用 hash，不在 GET 路径写库；seed 在开发/测试初始化或显式初始化流程执行；内置 seed `2026.07-v6.5-schema.2` 已带稳定 `sectionId/sourceId`；正式初始化会写入 `content_pages` 与 `content_page_versions`，要求 active admin 发布归因，默认不覆盖已有差异版本；回归覆盖中文目录/中文 SQLite 文件名下的迁移与初始化 |
...
| GET | `/api/render/script-sandboxes/{sandbox_id}/page/{slug}` | 返回内容脚本 sandbox HTML；按已发布私有 schema 定位 manifest，每次响应生成唯一 nonce，CSP `script-src` 与 bootstrap `<script nonce>` 同步绑定，missing/blocked/歧义/外部未镜像资产 fail closed |
