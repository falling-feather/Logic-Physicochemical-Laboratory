...
- 组织治理使用原生 dialog：busy 时 Escape 不关闭且焦点落到标题，空闲时 Escape 可关闭；成功、锁定和解锁分别移动到状态区或可操作入口，触发器被重建时重新定位。编辑、刷新和对话框操作目标至少 44×44，390×844 不产生根级水平溢出。当前角色静态资源、HTTP fallback、Service Worker 和缓存名称统一使用 `20260716v7426ModuleSwitchLifecycleP4`。
