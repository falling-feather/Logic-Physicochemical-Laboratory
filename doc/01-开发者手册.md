...
>
> ✅ **当前进度状态（2026-07-03）**：`houduan` 分支已新增 `backend/` FastAPI 后端骨架、数据库探针、Alembic 初始迁移、`/api/health`、本地账号、学校班级最小闭环和 `physics/energy-conservation` 内容协议持久化样例；既有前端路径保持可回滚。
...
│  - 后续 Auth / School / Class / Course / Assignment / Progress    │
└────────────────────────────────────────────────────────────────┘
```
...
FastAPI 后端位于 `backend/`，当前首切片提供配置、数据库探针、健康检查、内容协议样例和测试入口。
