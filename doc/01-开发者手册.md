...
>
> **当前基线**：V7.4.1 管理 API 用户/权限领域拆分首切片 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
...
| `app.services.admin_common` | V7.4.1 管理领域共享管理员角色校验、数据库计数、分页游标和审计变更快照；端点模块不得复制这些规则 |
| `app.api.endpoints.admin` / `admin_users` | `admin.py` 保持 `/api/admin` 聚合与尚未拆分领域的兼容入口；`admin_users.py` 承载 bootstrap、用户列表、角色/状态更新和管理员密码重置。新增拆分必须保持路由顺序、路径、响应 schema、状态码和审计 action 不变 |
