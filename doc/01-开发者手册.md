...
>
> **当前基线**：V7.5.10 已完成代码空间和未来星系课程化重绘、代码判题后端契约、教师编排前端、综合发布收束及独立终验：两个星系均使用 6 组课程目录、18 个独立子课和可完成互动；Alembic 0049 提供题目版本、学生源码提交、判题尝试/租约和授权分页查询；学生端以 Cookie 会话和内存态班级/课程映射消费权威发布计划，代码空间正式提交与未来星系直达裁剪均已接通。`#teacher` 在星序身份工作区以服务端分页管理三星系发布计划、学生进度和代码审阅。V7.5.8 完成性能/缓存/9001 收束，V7.5.9 补齐 44px 与方向键/Home/End roving 契约；QA-010 对精确提交使用真实 Edge 150 复验 PASS，P0/P1/P2/P3 均为 0。当前 `review` 从文档冻结提交 `8723dde196e70ea7ca5b1db8251dfbfa3a7736a0` 建立，正在执行增量全仓审查；默认 runner 关闭且 API 不执行学生源码，用户后置的域名与公网 R6 不因本机通过而自动恢复。
>
...
| 学生上下文 | `codevis/shared/js/student-context.js` | Cookie Session、本人班级与课程映射；单班级自动选定，多班级失败关闭 |
...
| 可执行挑战 | `codevis/pages/course-challenge/` | 预测、运行、追踪、修正、公开预检与权威正式提交入口 |
...
- V7.5.8 的两张未来星系首屏背景固定为 WebP，共 136,676 字节，不进入全局 APP_SHELL；本机预览必须返回精确 `image/webp`。课程目录使用自适应高度并允许页面纵向滚动，装饰层只在组件边界裁剪；轨道太阳/光源位于椭圆焦点，网络 Canvas 响应减少动效并限制设备像素比。
