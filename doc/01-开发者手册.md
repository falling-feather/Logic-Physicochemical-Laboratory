...
> **版本**: v6.0.0（项目层级调整 · 正式更名）  
> **文档大版本**: 6.x（覆盖 v5.0 / v5.1 全部内容 + v6.0 品牌升级）  
> **最后更新**: 2026-06-05 | **维护**: 星序 Dev Team
>
...
```
          ...
          ┌──────────┴──────────┐         ┌──────────┴──────────┐
          │  工科实验室          │         │  代码空间             │
          │  englab 星系          │         │  codespace 星系       │
          │  /index.html         │         │  /codevis/index.html │
          ├──────────────────────┤         ├──────────────────────┤
          │ 数学 · 物理 · 化学    │         │ JS · Python · C/C++  │
          │ 算法 · 生物           │         │ 三语言手写沙箱        │
          │ 71 个可视化实验       │         │ 代码执行追踪播放器     │
          │ Vanilla JS SPA       │         │ 独立 SPA + cv- 命名空间│
          └──────────────────────┘         └──────────────────────┘
                     ▲                               ▲
                     └────── pages/planets/ ─────────┘
                              （多星系顶层导航大屏）
...
| 维度 | 工科实验室（englab） | 代码空间（codespace / codevis 目录） |
|---|---|---|
| **角色** | 主站，承担学科可视化 | 独立子站，承担代码执行可视化 |
| **入口** | `index.html`（主 SPA） | `/codevis/index.html`（独立 SPA） |
| **命名空间** | 无前缀（`.btn` / `Router` / `CONFIG`） | `cv-` CSS 前缀 + `Cv*` 全局对象 |
| **依赖** | GSAP + Lucide + 自研 router | Lucide + 自研 router + 三语言沙箱 CDN |
| **数据流** | `shared/js/config.js` 集中配置 | 子站内部独立配置 |
| **路由** | hash 路由 + 学科页 + 模块选择器 | hash 路由（`#home` / `#trace`） |
| **Service Worker** | 主站 sw.js 缓存 | 由主站 sw.js 同样接管子站资源 |

### 顶层导航中转（planets 视图）

[pages/planets/planets.js](../pages/planets/planets.js) 渲染**多星系选择大屏**。顶层选择"星系" → 内部站内星系进入"学科行星"展开模式；外链星系（如代码空间）直接 `window.location.href` 跳转子站。  
...
15. [维护计划](#15-维护计划)
16. [更新日志](#16-更新日志)
17. [新增星系开发指南（v6.0 新增）](#17-新增星系开发指南v60-新增)

---

## 1. 项目概览

**星序 Astra** 是一个面向高中及大学初阶学生的纯前端**多星系**可视化学习平台。"星系"是项目的组织单位 —— 每个星系对应一个独立的学习方向、独立的视觉品牌、相对独立的技术栈。当前已上线两个星系：

- **工科实验室**（englab）：数学、物理、化学、算法、生物五大学科 71 个可视化交互实验，全面对齐人教版（2019 新课标）高中必修及选择性必修核心知识点。
...
│   │  (工科实验室星系)           │    │  (代码空间星系)           │  │
│   │  ─────────────────         │    │  ─────────────────       │  │
│   │  Router → Module Selector │    │  CvRouter → CvHome       │  │
│   │  → 71 Experiment Modules   │    │  → CodeTrace Player      │  │
│   │  + planets 顶层多星系导航   │    │  + JS/Py/C++ Runtimes    │  │
│   └─────────────┬─────────────┘    └────────────┬────────────┘  │
│                 │                               │               │
│                 │   ←  planets 跳转  →           │               │
│                 │                               │               │
│   ┌─────────────┴───────────────────────────────┴────────────┐  │
│   │  共享 shared/css/tokens.css 设计令牌（双子站均消费）       │  │
...
星序 Astra 在 v6.0 后正式确立"多星系"顶层架构。本节是从 0 到 1 创建一个新星系的完整参考。

### 17.1 决策矩阵：站内星系 vs 外链子站

| 维度 | 站内星系（主站新增板块） | 外链子站（独立 SPA） |
| --- | --- | --- |
| 适用场景 | 与现有学科同构（Canvas 2D 实验集合） | 技术栈/交互范式差异大（如代码执行、AI 推理、3D 场景） |
| 路由 | 共享 `shared/js/router.js` | 自带独立 router（如 `CvRouter`） |
| 部署 | 共享主站 `index.html` | 独立 `index.html`，独立目录（如 `codevis/`） |
| 命名空间 | 主站统一约定 | **必须**全套独立前缀（CSS class / JS 全局） |
| 与 planets 集成 | `CONFIG.galaxies` 配 `internal: true` | `CONFIG.galaxies` 配 `externalUrl: '/xxx/'` |
| 缓存 | 沿用主站 sw.js 缓存清单 | sw.js 缓存清单需追加子站静态资源 |
| 开发成本 | 低（复用现有基础设施） | 中-高（需建立独立 SPA + 隔离） |

**经验法则**：先尝试站内星系；只有当技术栈或交互范式与现有学科明显冲突时，才升级为外链子站。

### 17.2 需要绘制 / 准备的资产清单

1. **星系图标**：256×256 PNG 或矢量 SVG，用于 navbar 与 planets 大屏的星球贴图
...
4. **首页主星贴图**（可选）：仅外链子站需要自绘 Hero 主星
5. **学科卫星图标**：每个子模块一个图标，参考 muban/01 卫星轨道布局
...
| `Router.register(pageId, init, destroy)` | 页面生命周期 | 站内星系新增页面时调用 |
| `ModuleSelector.renderGallery(subject)` | 画廊渲染 | 学科页面通用 |
| `pages/planets/planets.js` 自动布局 | 星系球公转位置计算 | 不必动 JS，仅追加 CONFIG |
| `sw.js` 缓存策略 | stale-while-revalidate | 仅需在 `STATIC_ASSETS` 数组追加资源路径 |
| `shared/css/tokens.css` 设计令牌 | 颜色 / 间距 / 字体 / 阴影 | 通过 CSS 变量引用 |
| `muban/01~04` UI 模板 | 主页 / 导航 / 子站 / 画廊骨架 | 复制 HTML + CSS 起手即用 |
...
1. **主站 navbar / footer 的全局 HTML** — 外链子站必须自己写一套，不可 iframe 嵌入
2. **CSS 类名命名空间** — 例如代码空间用 `cv-` 前缀，新星系必须有自己的 2~3 字母前缀（如 `sg-` 智能星系 / `hg-` 历史星系）
...
1. **配置**：`shared/js/config.js` → `CONFIG.galaxies` 追加 `{ id, name, color, subjects: [...] }`
2. **目录**：`pages/{galaxy}/` 新建文件夹 + `{galaxy}.css` + `{galaxy}.js`
3. **HTML 骨架**：`index.html` 追加 `<section data-module="{galaxy}">`（参考 muban/04 画廊布局）
4. **实验模块**：每个实验一个 JS 文件，封装为对象（`init` / `destroy`）
5. **脚本引用**：`index.html` 底部 `<script defer src="pages/{galaxy}/xxx.js">`
6. **路由注册**：`router.js` 的 `onPageEnter` 内调用 `initXxx()` / `destroyXxx()`
7. **首页卫星**：`pages/home/home.js` 卫星轨道追加一颗（color + label + targetHash）
8. **planets 大屏**：自动从 `CONFIG.galaxies` 读取，无需手改
9. **CSS 令牌**：`shared/css/tokens.css` 追加 `--{galaxy}-primary` / `--{galaxy}-accent`
10. **sw.js**：`STATIC_ASSETS` 追加新 JS / CSS 路径，bump 缓存版本
...
9. **navbar 跳转**：主站 navbar 增加"返回多星系"链接（参考 codevis/index.html footer 实现）
10. **部署**：`deploy.ps1` 与 httplib 服务器自动覆盖（静态资源透传，无需后端改造）

### 17.7 验收清单（上线前必查）

- [ ] `CONFIG.galaxies` 配置正确，planets 大屏可见新星系
- [ ] 首页卫星轨道 / navbar 入口可点击进入
...
- [ ] doc/02-更新规划.md 移除该星系任务、doc/16 更新日志追加版本条目
