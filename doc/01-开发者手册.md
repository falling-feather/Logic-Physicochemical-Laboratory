...
>
> **当前基线**：V7.5.6 已完成代码空间和未来星系课程化重绘及代码判题后端契约：两个星系均使用 6 组课程目录、18 个独立子课和可完成互动；Alembic 0049 另提供题目版本、学生源码提交、判题尝试/租约和授权分页查询。V7.5.3 的三星系通用发布计划和 V7.4.37 的星序角色层级纠偏继续有效。`#student/#teacher/#admin` 仍统一登记为 `astra` 角色工作区，与三个学习单元共同归属星序；浏览器预检不是权威判题，正式提交 UI 与教师编排等待 FE-010。默认 runner 关闭且 API 不执行学生源码。用户已把域名与公网 R6 后置，本机结论不替代目标环境证据。
>
