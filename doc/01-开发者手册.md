...
[pages/planets/planets.js](../pages/planets/planets.js) 驱动**星序总览页**的身份文案、角色入口和 disclosure 生命周期。V7.4.32 起，三个星系及其二级资源在同一张开放式轨道索引中同时可达，不再依赖持续旋转的 Canvas 拨盘；工科/未来资源继续走主站 hash，代码空间直接进入独立子站。页面只显示真实目录和服务器会话身份，不用占位数字冒充学习进度。
