...
>
> **当前基线**：V7.5.10 已完成两个课程化星系、代码判题契约、教师编排、综合收束及 QA-010；当前 `review` 候选已执行 `re1`—`re14` 增量审查。Alembic 0050 在 0049 四类 OJ 模型上增加有界过期租约恢复索引；学生写入与课程发布更新使用同事务锁定门禁，多班级作业历史要求显式班级，高基数成员/提交/事件/尝试使用分页接口，旧数组最多兼容 200 项。`#teacher` 使用服务端分页，并按 foundation/workbench/curriculum 三层顺序加载角色样式；代码空间终态轮询、未来课程初始化/互动证据也已修正。默认 runner 关闭且 API 不执行学生源码，review 仍待 QA-011；域名、公网 R6、真实 MySQL 和隔离 runner 不因本机通过而自动恢复。
>
