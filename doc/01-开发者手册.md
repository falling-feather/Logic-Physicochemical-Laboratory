...
>
> **当前基线**：V7.4.21 已恢复 09 号现行后端阶段追踪 + V7.4.20 已修复工科首页 390px 根级横向溢出 + V7.4.19 已修正 DNA/遗传模式 wrapper 真实 DOM 挂载 + V7.4.18 已验证 23 项实验整页离开 cleanup、补齐 2 项缺失取消协议 + V7.4.11 Python/Node/C++ 依赖与产物追踪完成 + V7.4.8 管理 API 全部分域拆分完成 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
...
> 2. 业务 API 只进入 `backend/`；认证保持 HttpOnly cookie-only，页面与实验保持匹配的 init/destroy 生命周期。
...
4. `AstraPageRegistry` 提供当前页面的脚本与 ready/enter/leave 钩子，Router 只在角色守卫通过后补加载；`Router.galaxySupportScripts` 只为当前星系补轻量 support
...
- V7.3.1 起，完成 `npm ci --ignore-scripts` 后，三角色发布前业务证明统一运行 `node tools/browser/role-workflows-proof.cjs --api <local-api> --web <local-web> --confirm-isolated-environment`。脚本会创建管理员/教师/学生和教学数据，只允许本地 URL、要求 development/test/testing 环境与一次性数据库；真实目标环境不得传入该脚本。验收报告与截图写入被忽略的 `test-screenshots/`。
...
- cleanup 状态使用 `legacy-callback`、`candidate-unwired`、`validated-callback` 或 `missing`。只有完成真实 owner 核对、可取消资源修复和离开/重入验证的条目才能配置 `validated-callback / verified:true / run`；空函数、错误 owner 和未经验证的候选均不得伪装成可执行 cleanup。V7.4.19 当前仍为 `65 legacy-callback + 23 validated-callback + 0 missing`。
...
- 整页离开顺序固定为关闭活动实验、关闭公共实验工具、关闭物理/生物学科级 Zoom、执行注册表 cleanup、重置 ModuleSelector 状态；`openModule()` 与 `closeModule()` 当前不执行 cleanup，实验切换/返回画廊的统一语义必须等待后续任务验证。
...
5. **实现并验证生命周期后升级 cleanup 状态**：先提供真实 destroy/取消协议，再通过 owner 契约和离开/重入证据将 `missing` 或 `candidate-unwired` 升级为 `validated-callback / verified:true`；Router 不再登记实验 destroy。
6. **测试**：运行 `experiment-registry-contract.cjs`，启动服务器，访问对应学科页面 → 点击卡片进入实验 → 离开整页再重入；检查桌面/390×844、Canvas、重复控件与 console。
