...
> **文档定位**：长期维护星序的架构、模块边界、开发规则与扩展方式；版本流水和完成证据见 `03-发布历史.md`，未来任务见 `02-更新规划.md`。
>
> **当前基线**：V7.4.4 管理 API 知识快照任务拆分 + V7.4.3 内容治理领域拆分 + V7.4.2 组织领域拆分 + V7.4.1 用户/权限领域拆分 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
> **最后更新**：2026-07-11　**维护者**：星序 Dev Team
>
> **文档体系**：[02-更新规划.md](02-更新规划.md) · [03-发布历史.md](03-发布历史.md) · [04-部署指南.md](04-部署指南.md) · [05-UI规范模板.md](05-UI规范模板.md) · [06-实验体验与信度审查报告-20260606.md](06-实验体验与信度审查报告-20260606.md) · [07-后端优化与设计.md](07-后端优化与设计.md) · [08-前端页面实现索引.md](08-前端页面实现索引.md) · [99-历史审视报告归档.md](99-历史审视报告归档.md)。
>
> **层级硬规则**：星序是顶层品牌，星系是一级入口，星系内学科/知识目录是二级入口。新增星系必须有独立 UI 外壳，不得直接复制工科试验室的顶栏、侧边栏、学科页或页脚。
...
V7.4.0 起采用**认证前置应用壳 + 星系级按需加载**策略。加载屏先等待 `api-client` 与 `app-session` 完成会话判定；只有认证成功后才启动 Router，不等待全部星系页面、实验模块、学习框架或 Service Worker 缓存完成。
...
2. `app-session.js` 先调用 `/api/users/me`：401 时锁定主应用并呈现统一账号入口，认证成功时写入非敏感内存态并裁剪导航/section
3. `main.js` 仅在会话确认后调用 `initApp()`；Router 对当前 hash 做角色守卫，再进入允许页面
4. `Router.pageScripts` 只为当前且有权访问的页面补加载页面脚本；`Router.galaxySupportScripts` 只为当前星系补轻量 support
5. `ModuleSelector._moduleScripts` 只在用户打开具体实验时加载该实验 JS
...
| `app.services.admin_common` | 管理领域共享管理员角色校验、数据库计数、分页游标、转义搜索模式、UTC 比较归一化、审计变更快照和待批改提交状态口径；端点模块不得复制这些规则 |
...
| `app.api.endpoints.admin` / `admin_users` / `admin_organizations` / `admin_content` / `admin_snapshot_tasks` | `admin.py` 保持 `/api/admin` 聚合与尚未拆分领域的兼容入口；子模块分别承载用户/权限、组织、内容治理和知识快照任务。新增拆分必须保持路由顺序、路径、响应 schema、状态码和审计 action 不变 |
...
- V7.3.1 起，三角色发布前业务证明统一运行 `node tools/browser/role-workflows-proof.cjs --api <local-api> --web <local-web> --confirm-isolated-environment`。脚本会创建管理员/教师/学生和教学数据，只允许本地 URL、要求 development/test/testing 环境与一次性数据库；真实目标环境不得传入该脚本。验收报告与截图写入被忽略的 `test-screenshots/`。
- V7.3.2 起，真实 staging/production 完成度统一由 `python -m scripts.target_release_gate --manifest <bundle>/target-release.json` 判定。该闸只校验证据，不配置域名、证书、防火墙、secret、数据库或监控；缺少外部 owner/审批或任一哈希证据必须返回延期。模板见 `backend/target-release-manifest.example.json`，真实证据包与 secret 不得提交仓库。
...
- V7.4.0 新增用户、学校、班级分页面板和领域数据地图。地图只展示经 API 汇总的实体规模、角色分布与数据库健康，不返回数据库 URL，也不提供任意表/SQL 编辑；用户角色/状态变更必须双确认，并保留服务端最后管理员保护、活动会话撤销、审计与权威对账。
...
| V6.6.37 至 V6.6.63 后端阶段收束结果 | `03-发布历史.md` |

### 15.1 当前开发口径

- `main` 是集成与发布主线；`houduan` 已完成 V6.6.63 后端阶段，后续 feature/review 分支应短生命周期并在门禁通过后回流主线。
...
本手册不再复制版本流水。所有已完成版本、历史修复、review 结论和验证证据统一维护在 [`03-发布历史.md`](03-发布历史.md)；下一阶段任务只维护在 [`02-更新规划.md`](02-更新规划.md)。
