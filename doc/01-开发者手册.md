...
>
> ✅ **当前进度状态（2026-07-03）**：`houduan` 分支已新增 `backend/` FastAPI 后端骨架、数据库探针、Alembic 迁移、`/api/health`、部署预检、部署 smoke、API no-store 缓存边界、本地账号认证安全基线、学校班级最小闭环与加入申请审批、课程/作业/学习事件/提交批改/积分统计、知识状态/班级规则统计、个人/班级知识快照、知识快照周期重算脚本/运行记录/进程内调度器、管理端 API、管理端加入申请队列、管理端列表分页搜索、审计元数据与认证事件审计、`physics/energy-conservation` 内容协议持久化样例、ContentDraft 草稿与脚本审核后端切片和前端 opt-in schema 渲染试点；既有前端路径保持可回滚。
...
FastAPI 后端位于 `backend/`，当前首切片提供配置、数据库探针、健康检查、部署预检、部署 smoke、API no-store 缓存边界、内容协议样例、内容 seed 启动初始化与读取无副作用边界、ContentDraft 草稿与脚本审核、本地认证安全基线、学校班级加入申请审批、学校/班级/课程访问控制服务层、课程/作业/学习事件、提交批改、积分流水、个人进度摘要、知识状态/班级规则统计、个人/班级知识快照、知识快照周期重算脚本/运行记录/进程内调度器、管理端基础 API、管理端加入申请队列、管理端列表分页搜索、审计元数据与认证事件审计、前端 opt-in schema 试点和测试入口。
...
| `app.services.content_catalog` | 内容页 seed、schema 读取和内容页摘要；GET 查询只读现有记录，seed 在开发/测试初始化或显式初始化流程执行 |
