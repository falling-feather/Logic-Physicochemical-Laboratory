...
**星序 Astra** 是一个面向高中及大学初阶学生的纯前端**多星系**可视化学习平台。"星系"是项目的一级组织单位 —— 每个星系对应一个独立的学习方向、独立的视觉品牌、相对独立的技术栈。星系内部的学科或知识方向才是二级目录。
...
- 纯前端渲染，无构建步骤、无框架依赖（Vanilla JS + Canvas 2D）
...
FastAPI 后端位于 `backend/`，当前首切片提供配置、数据库探针、健康检查、部署预检、API no-store 缓存边界、内容协议样例、本地认证安全基线、学校班级加入申请审批、课程/作业/学习事件、提交批改、积分流水、个人进度摘要、知识状态/班级规则统计、个人/班级知识快照、知识快照周期重算脚本/运行记录/进程内调度器、管理端基础 API、管理端加入申请队列、管理端列表分页搜索、审计元数据与认证事件审计、前端 opt-in schema 试点和测试入口。
...
- [X] Windows 云服务器部署准备（deploy.ps1 + 04-部署指南.md）
