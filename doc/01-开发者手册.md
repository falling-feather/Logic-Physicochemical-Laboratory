...
>
> **当前基线**：V7.4.34 已完成 `127.0.0.1:9001` 同源本机入口、静态白名单、哈希锁环境、Alembic 迁移和可选首管理员交互引导 + V7.4.33 已统一三角色登录后星序落点、独立角色工作台语义和星序安全退出 + V7.4.32 已完成开放式三星系资源总览和单一全局治理台视觉重组 + V7.4.30 已补齐 staging/production 服务包环境语义 + V7.4.29 已补齐 Windows DPAPI 受保护密钥存储、服务账号绑定、精确 HTTPS origin 服务包参数和目标 staging 浏览器证明入口 + V7.4.28 已完成 target-release-v2/51 项目标证据闸 + V7.4.27 已由 QA-007 使用真实 Edge、一次性 SQLite 和隔离账号关闭 R5 当前构建三角色终验，覆盖匿名登录/SW 前置、三角色资源矩阵、业务与越权、桌面/390×844、5 张截图及同库只读总账 + V7.4.26 已在 QA-005 通过后关闭 R4b，只对 23 项 `verified:true` 实验启用精确模块级 cleanup，65 项 legacy 保持 fail closed，0 项 missing + V7.4.25 已完成管理组织权威状态可视化、受限编辑/归档对话框、歧义写入对账锁和归档教学只读边界 + V7.4.24 已完成学校/班级受约束治理、Alembic 0047 与活动组织写入/当前统计边界 + V7.4.23 三角色 CSS/JS 资源级裁剪已通过 QA-008 独立验收 + V7.4.22 浏览器 Cookie-only/非浏览器 Bearer 双通道 fail-closed 契约 + V7.4.21 已恢复 09 号现行后端阶段追踪 + V7.4.20 首页 390px 根级横向溢出已由 QA-006 独立通过 + V7.4.19 已修正 DNA/遗传模式 wrapper 真实 DOM 挂载 + V7.4.18 已验证 23 项实验整页离开 cleanup、补齐 2 项缺失取消协议 + V7.4.11 Python/Node/C++ 依赖与产物追踪完成 + V7.4.8 管理 API 全部分域拆分完成 + V7.4.0 登录前置与角色应用外壳 + V6.6.63 后端阶段完成。R1—R5b 已形成仓库发布候选；用户已把域名与公网 R6 后置，当前进入 QA-009 的 9001 三角色集中验收。
>
