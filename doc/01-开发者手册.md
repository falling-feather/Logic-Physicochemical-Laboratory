...
>
> **当前基线**：V7.4.27 已由 QA-007 使用真实 Edge、一次性 SQLite 和隔离账号关闭 R5 当前构建三角色终验，覆盖匿名登录/SW 前置、三角色资源矩阵、业务与越权、桌面/390×844、5 张截图及同库只读总账 + V7.4.26 已在 QA-005 通过后关闭 R4b，只对 23 项 `verified:true` 实验启用精确模块级 cleanup，65 项 legacy 保持 fail closed，0 项 missing + V7.4.25 已完成管理组织权威状态可视化、受限编辑/归档对话框、歧义写入对账锁和归档教学只读边界 + V7.4.24 已完成学校/班级受约束治理、Alembic 0047 与活动组织写入/当前统计边界 + V7.4.23 三角色 CSS/JS 资源级裁剪已通过 QA-008 独立验收 + V7.4.22 浏览器 Cookie-only/非浏览器 Bearer 双通道 fail-closed 契约 + V7.4.21 已恢复 09 号现行后端阶段追踪 + V7.4.20 首页 390px 根级横向溢出已由 QA-006 独立通过 + V7.4.19 已修正 DNA/遗传模式 wrapper 真实 DOM 挂载 + V7.4.18 已验证 23 项实验整页离开 cleanup、补齐 2 项缺失取消协议 + V7.4.11 Python/Node/C++ 依赖与产物追踪完成 + V7.4.8 管理 API 全部分域拆分完成 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。R1—R5 现为仓库发布候选，R6/OPS-001 仍等待真实目标环境输入。
>
> **当前任务入口**：项目对接、目标环境发布与前端注册表任务只在 [`02-项目规划.md`](02-项目规划.md) 维护；本文不复制任务状态。
>
> **具体规划法则**：
>
> 1. 星序是顶层品牌，星系是一级入口，星系内学科/知识目录是二级入口；新增星系必须有独立 UI 外壳。
...
├── package-lock.json           # Playwright/core/fsevents 完整 integrity 锁
│
...
2. `app-session.js` 先调用 `/api/users/me`：401 时锁定主应用并呈现统一账号入口；成功时按 `AstraPageRegistry` 的角色矩阵清理旧缓存、加载允许的角色样式，再写入非敏感内存态并裁剪导航/section
...
- 新后端开发应先阅读 `02-项目规划.md` 的任务/版本、`09-后端阶段收束小版本开发安排.md` 的执行顺序与退出门禁、本文的服务边界和 `07-后端优化与设计.md` 的架构/风险表；历史 V6.6 完成项以 `03-发布历史.md` 为准，V7.4.12 起完成事实以 `03-开发历史.md` 为准。
...
- V7.4.27 起，三角色发布前业务证明统一运行 `node tools/browser/role-workflows-proof.cjs --api <local-api> --web <local-web> --out <empty-proof-dir> --channel msedge --confirm-isolated-environment --workflow-timeout-ms 900000`。脚本只允许本地 URL、development/test/testing 环境、一次性空数据库和空证据目录，并把 Git HEAD、完整 dirty 集合、关键文件 SHA-256、真实 Edge channel 与 Service Worker 代际写入报告；真实目标环境不得传入该脚本。
...
- 浏览器报告 `ok=true` 后才可运行 `python tools/quality/sqlite-qa-ledger.py --database <same-isolated-db> --output <proof-dir>/sqlite-ledger.json`；helper 使用 SQLite `mode=ro`，核对全部表计数、Alembic 版本、审计链头和 `admin-authority` 单例锁，并以排他临时文件、fsync 和原子替换写报告，拒绝输出覆盖数据库或其硬链/符号链接。当前结论仍只证明一次性 SQLite 与本机真实 Edge，不替代 MySQL、staging 或 R6 目标环境发布证据。
...
- 组织治理使用原生 dialog：busy 时 Escape 不关闭且焦点落到标题，空闲时 Escape 可关闭；成功、锁定和解锁分别移动到状态区或可操作入口，触发器被重建时重新定位。编辑、刷新和对话框操作目标至少 44×44，390×844 不产生根级水平溢出。当前角色静态资源、HTTP fallback、Service Worker 和缓存名称统一使用 `20260716v7427RoleWorkflowGateP0`。
...
- `ModuleSelector.openModule(page, moduleId)`：先预校验目标；A→B 时先关闭学科 Zoom，再 cleanup 当前已验证 owner，成功后提交新的 transition generation、隐藏旧实验、显示目标并重新 init。Zoom/owner 失败时不修改 DOM、hash 或 active state。
- `ModuleSelector.closeModule(page)`：B→画廊时使用相同的 Zoom/cleanup 事务；成功后清除已验证 owner 的初始化标记并恢复画廊，legacy 保留初始化标记。
- `ModuleSelector.leavePage(page)`：整页离开使用注册表 `cleanupPage()` 做兼容清理，关闭公共工具并重置 dirty/init/generation 状态；调用方通过 `skipExperimentCleanup` 避免已执行的模块级 cleanup 与页面级 cleanup 双调用。
...
| 信息类型 | 维护位置 |
| -------- | -------- |
| 下一阶段任务、优先级、版本、验收标准和不做事项 | `02-项目规划.md` |
| 后端重构阶段顺序、退出门禁和 V6.6 历史锚点 | `09-后端阶段收束小版本开发安排.md` |
| V7.4.12 起已完成版本、验证证据、剩余风险和历史复盘 | `03-开发历史.md` |
...
- 当前长期集成分支为 `houduan`，由主开发单写并按 02 分配 V7.4.x；长期 FE/OPS/QA 默认只读，其他 feature/review 工作树只有在明确写锁和版本令牌下才能写入，门禁通过后由主开发集成。
...
- 每轮完成后，V7.4.12 起的验证证据写入 `03-开发历史.md`；V7.4.11 及以前的历史证据保留在 `03-发布历史.md`。规划文档只保留仍影响下一步决策的事项。

---

## 16. 发布历史入口

本手册不再复制版本流水。V7.4.11 及以前的历史修复和证据维护在 [`03-发布历史.md`](03-发布历史.md)，V7.4.12 起完成事实维护在 [`03-开发历史.md`](03-开发历史.md)；下一阶段任务只维护在 [`02-项目规划.md`](02-项目规划.md)，后端阶段门禁维护在 [`09-后端阶段收束小版本开发安排.md`](09-后端阶段收束小版本开发安排.md)。
...
- [ ] `doc/02-项目规划.md` 更新该星系任务状态，`doc/03-开发历史.md` 追加完成记录
