...
| `app.services.content_catalog` | 内容页 seed、正式内容初始化、published schema 读取和内容页摘要；GET 查询只读当前已发布记录并剥离原始脚本引用，只保留不可执行 `scriptManifest`，seed 在开发/测试初始化或显式初始化流程执行；正式初始化会写入 `content_pages` 与 `content_page_versions`，要求 active admin 发布归因，默认不覆盖已有差异版本；回归覆盖中文目录/中文 SQLite 文件名下的迁移与初始化 |
| `app.services.content_script_policy` | 内容草稿脚本静态分析与 sandbox 契约；输出 policy version、schema hash、风险等级、sandbox 摘要和 findings，阻断危险协议、路径穿越、内联脚本、`<script>` 标签、缺失 sandbox 与不安全 sandbox 能力；当前不承担浏览器 iframe/worker 运行时执行 |
...
正式环境不依赖启动时自动 seed。初始化脚本默认先跑部署预检，只允许 active admin 归因；内置页面带脚本引用时，非 dry-run 必须显式传入 `--allow-reviewed-scripts`。若已有当前版本与内置 schema 不同，默认报告冲突；确认以新版本推进时再使用 `--upgrade-existing`，且活跃草稿默认会阻断升级。
...
| GET | `/api/content/pages/{slug}` | 返回内容协议详情；公开响应剥离原始脚本引用，只保留不可执行 `scriptManifest` |
| POST | `/api/content/drafts` | 教师或管理员创建内容草稿；草稿不进入公开渲染，并记录当前 published base 版本、schema hash 与脚本静态分析结果；同一作者同一目标页只允许一个 active 草稿 |
...
| PATCH | `/api/content/drafts/{id}/script-review` | 管理员审核允许脚本的草稿，作者不能自审；阻断级脚本 policy 不能审核通过 |
| POST | `/api/content/page-versions/{id}/rollback` | 管理员按历史版本追加式回滚，生成新的当前版本；带脚本 policy findings 的历史版本必须新建草稿重审 |
| GET | `/api/render/page/{slug}` | 返回前端可渲染页面结构；公开响应剥离原始脚本引用，只保留不可执行 `scriptManifest` |
