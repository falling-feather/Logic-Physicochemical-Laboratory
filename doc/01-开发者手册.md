...
> ✅ **V6.6.59 增量（2026-07-10）**：完成安全/隐私冻结。非本地环境默认 production-like，admin bootstrap 必须配置 token 或显式关闭；CORS 仅允许精确 origin。全局角色与学校/班级/课程权限行联动校验，角色变化撤销会话并清理不兼容权限；admin 权限控制面使用数据库锁串行化。内容审核绑定最后编辑者/精确 schema hash 并限制 schema 预算；外部脚本统一使用公网 HTTPS pinned fetch，sandbox 资产按 hash CSP/SRI 执行且导航 fail closed。敏感学习状态不再持久化到普通浏览器 storage，静态服务器不再公开仓库根目录；Alembic head 为 0045。
>
> ✅ **V6.6.60 增量（2026-07-10）**：本地发布候选总验收完成并判定延期。opaque sandbox 的 hash-bound SRI 资产使用资源级匿名 CORS，外部 Edge 桌面/390×844 proof 27/27；C++ 静态服务删除 `/api/info` 和占位 `/api/eval`，只保留内部 health。SQLite 空库迁移/preflight/smoke 与 MySQL-required fail-closed 通过；真实 MySQL、反代/服务注册和合格 C++17 工具链进入 V6.6.61-V6.6.62。
>
> ✅ **V6.6.61 增量（2026-07-10）**：隔离、仅回环 MySQL 8.0.46 完成 0001→0046、preflight/smoke、内容/任务/调度/审计并发、代表性数据 EXPLAIN ANALYZE、连接池压力、0043 DDL 和独立备份恢复，关闭 `RC-MYSQL`。
>
> ✅ **V6.6.62 增量（2026-07-11）**：真实 Caddy 分流与 WinSW 四服务拓扑完成实机演练；四服务均使用 `LocalService`、回环监听、自动延迟启动和失败重启。MSVC 19.43 Release 构建、API 故障静态回退、服务/PID 恢复、代理配置与静态二进制回滚、直连端口隔离通过，stage gate 14/14。公开 `/api/health` 不再返回数据库地址。
>
> **品牌**: 星序 Astra · 多星系可视化学习平台  
> **版本**: V6.6.62 真实拓扑与 Release 构建实证；Python 业务后端与 C++ 静态服务处于并存过渡期
> **文档大版本**: 6.x（覆盖 v5.0 / v5.1 / v6.x 多星系架构）
> **最后更新**: 2026-07-11 | **维护**: 星序 Dev Team
>
...
>
> ✅ **当前进度状态（2026-07-11）**：`houduan` 分支已形成 FastAPI 业务后端、连续迁移、内容/认证/教学/治理 API、三端入口、内容脚本隔离执行链和生产试运行门禁。V6.6.61-V6.6.62 已关闭真实 MySQL、真实拓扑和合格 C++17 Release 构建 P1，生产 stage gate 14/14；当前进入 V6.6.63，只处理明确纳入首个 RC 的外部通道与长期证据，未选中能力继续默认关闭。完整历史能力与验证证据见 `03-发布历史.md`。
...
> ✅ **V6.6.53 增量（2026-07-10）**：班级成员新增同校转班与用户名批量导入，学生移除冻结为保留历史的软停用；课程协作者扩展为 `editor/content_editor/assessment_editor/viewer` 并支持批量 upsert；作业新增 `audience_mode` 与 `AssignmentClassPolicy`，可按班级覆盖 assigned、状态、截止时间和积分规则。学习分析升级为 `rule_version=v2`，以 effective active assignment-class pair 为分母，稳定输出 course/unit/knowledge_point/assignment 维度并兼容读取 v1 快照；教师端、学生端和管理端加入相应操作/解释入口。

> ✅ **V6.6.54 增量（2026-07-10）**：新增 `app.services.alert_delivery` 与 `POST /api/admin/alert-outbox/dispatch-plans/{id}/dispatch`。外部通道默认关闭；启用时要求 HTTPS URL 和 SecretStr token 由环境/安全配置注入，admin 显式确认且 plan 再校验通过后才逐项投递。请求带 Bearer、HMAC-SHA256 和稳定 Idempotency-Key，信封不含原始 payload/source key；失败项进入 `failed` 并可人工重新排队，来源业务事务不受影响。

> ✅ **V6.6.55 增量（2026-07-10）**：新增 `background_tasks/background_task_attempts`、`app.services.background_tasks`、`app.services.background_task_worker` 与 `scripts.run_background_tasks`。统一任务控制面支持持久化调度入队、原子租约、heartbeat、attempt、指数退避、dead-letter、retry/cancel 和过期接管；知识快照/脚本扫描继续以领域 run 去重，告警沿用稳定外投幂等键。worker 和内容脚本外网执行均默认关闭，管理响应不返回 payload 或 lease token。

> ✅ **V6.6.56 增量（2026-07-10）**：新增 `audit_chain_heads/audit_archive_anchors`、Manifest v2、`app.services.audit_anchor_delivery`、`app.services.audit_archive_anchors` 与 `scripts.anchor_audit_archive`。MySQL 以链尾行锁串行化审计写入，SQLite 本地回归以事务级进程锁验证；外部锚定只发送 hash/范围摘要，失败走统一任务退避且不改写原链/归档。源删除仍禁止，脱敏必须派生新归档，恢复需外部回执复验、双人变更单和已验证备份。
> ✅ **V6.6.57 增量（2026-07-10）**：新增 `app.services.external_issue_providers`、`app.services.bug_external_sync` 与 `bug_external_sync_operations`。管理员可手工绑定外部 URL/编号，或显式确认 GitHub issue 创建、状态和安全评论同步；稳定操作键阻止重复副作用，创建/评论未知结果进入 ambiguous 且不盲重试。外部系统不反向覆盖本地 `BugRecord`，自动外发不包含 evidence、notes、source、URL 或凭据；Gitee/Jira 仅预留协议扩展位。
> ✅ **V6.6.58 增量（2026-07-10）**：新增 `app.services.backend_performance`、`scripts.backend_performance_drill`、管理端性能报告和 Alembic 0043。11 个 profile 覆盖审计/知识/脚本扫描/提交/任务/Bug 查询，复核 10 个新复合索引与既有 claim 索引，并执行不回传 SQL/参数/结果值的 EXPLAIN 和有界基准。API 返回 `Server-Timing`，慢日志只保存安全元数据；MySQL pool/overflow/wait/recycle/connect/read/write timeout 显式配置，业务写事务不自动重试。
...
├── README.md                   # 项目简介
├── deploy.ps1                  # Windows 四服务演练包生成入口；不下载工具、不开放端口、不安装服务
│
...
│   ├── 09-后端阶段收束小版本开发安排.md  # V6.6.37-V6.6.63 后端收束路线图
...
- 生产级缺口仍包括真实 MySQL、真实反向代理/服务注册与重启恢复、standalone 浏览器证明脚本在当前运行环境的完整重跑、强设备绑定、长期会话策略、正式告警和审计外部归档；邮件/短信/MFA 投递标记为 `P4 / 最低优先级 / 暂缓`。
...
| `ASTRA_CORS_ORIGINS` | 允许凭据型 API/CSP frame ancestor 的精确 origin，默认本地两个来源；禁止 `*`、`null`、userinfo、路径、query 和 fragment |
...
- 首轮读取 `/api/users/me`、`/api/admin/stats`、脚本/知识快照/outbox health/queue 和主要治理列表；所有列表必须传 `limit/offset`，UI 不应一次性拉取无界数据。V6.6.53 已将单条加入申请批准/拒绝作为第一项可执行治理动作产品化。
- 管理写操作必须经过确认、禁用自动重试，并在结果未知或成功后刷新失败时保持入口锁定，直至全局刷新完成权威对账。脚本 Host policy、outbox 复核、dispatch plan、快照 cancel/requeue、Bug 更新等其余动作仍须逐项完成相同门禁后再开放。
...
- 班级范围以 `GET /api/classes?mine=true` 的 active membership 为准。没有班级时不展示学校/班级公共目录，只允许学生输入教师提供的 `class_id` 调用 direct join；入班后重新读取本人班级和作业中心。“教师提供”当前只是 UI/运营约定，后端不验证 ID 来源，不能当作邀请码安全边界；正式邀请模型与可猜 ID 风险仍需后续单独设计。
...
脚本只生成 `EngLab/AstraApi/AstraWorker/AstraProxy` 四服务定义、回环 Caddyfile、hash 和可逆命令，不联网下载、不开放端口、不安装或启动服务。审查、ACL、正式 80/443/TLS 和安装顺序以 04 号文档为准。
...
- 学校、班级、课程范围判断优先复用 `app.services.access_control`。课程 owner 转让仍只允许 current owner 或全局 admin；课程协作者支持 `editor/content_editor/assessment_editor/viewer` 与 owner/admin 批量 upsert，分别约束结构、作业/评估、积分规则和只读能力，但任何课程角色都不会自动授予班级成员、挂课、提交、评分或学情权限。作业通过 `audience_mode` 与 `AssignmentClassPolicy` 按班级覆盖 assigned、状态、截止时间和积分规则；班级策略写入同时要求课程编辑能力与目标班级 active teacher scope。外部 issue 只做管理员显式出站同步，本地 `BugRecord` 是权威源，任何入站/双向覆盖必须另行设计和审计。剩余真实 MySQL、性能/安全与发布候选门禁按 `02-更新规划.md` 和 `09-后端阶段收束小版本开发安排.md` 推进。
- 性能报告只作为本地合同与真实环境留证入口：SQLite 允许验证索引、分页和输出脱敏，但 MySQL `EXPLAIN ANALYZE`、建索引 DDL、buffer pool/filesort、锁等待、深分页及 worker/API 并发必须在 V6.6.61 真实 MySQL 环境重新执行。worker 推荐独立服务，不能用应用层自动重试掩盖写事务结果不确定性。
