...
| 服务 | 职责 |
| ---- | ---- |
| `app.services.access_control` | 学校、班级、课程可见性与角色范围判断；普通业务端点应复用它，避免 `_require_*` 逻辑漂移；教师端班级挂课、学习事件、积分流水、学生进度、作业提交列表与批改必须落到 active class teacher scope；课程结构写入要求课程作者或全局 admin；全局 admin 保留跨范围治理能力 |
...
| GET | `/api/admin/schools/{id}/stats` | 管理端学校深度统计 |
| GET | `/api/admin/classes` | 管理端班级基础查看；支持分页、学校过滤与关键字搜索 |
| GET | `/api/admin/classes/{id}/stats` | 管理端班级深度统计 |
...
| GET | `/api/admin/submissions/pending` | 管理端待批改队列，可按学校、班级、课程、作业、学生、状态和时间窗过滤 |
...
- 学校、班级、课程范围判断优先复用 `app.services.access_control`；管理端统计拆分、权限矩阵、MySQL 联调和前端 schema 自动回归按 `02-更新规划.md` 推进。
