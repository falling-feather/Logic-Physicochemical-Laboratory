...
>
> **当前基线**：V7.5.4 已完成代码空间课程化重绘，默认路径为 6 组课程目录、18 个独立子课与可执行挑战；V7.5.3 的三星系通用发布计划和 V7.4.37 的星序角色层级纠偏继续有效。`#student/#teacher/#admin` 仍统一登记为 `astra` 角色工作区，与工科试验室、代码空间、未来星系三个学习单元共同归属星序；代码空间的浏览器预检不是权威判题，正式提交等待 BE-005。用户已把域名与公网 R6 后置，本机结论不替代目标环境证据。
>
