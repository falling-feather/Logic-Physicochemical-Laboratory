...
>
> **当前基线**：V7.4.17 已验证 21 项实验整页离开 cleanup + V7.4.11 Python/Node/C++ 依赖与产物追踪完成 + V7.4.8 管理 API 全部分域拆分完成 + V7.4.0 登录前置与角色应用外壳 + V7.3.2 目标环境发布证据闸 + V6.6.63 后端阶段完成。
>
...
- cleanup 状态使用 `legacy-callback`、`candidate-unwired`、`validated-callback` 或 `missing`。只有完成真实 owner 核对、可取消资源修复和离开/重入验证的条目才能配置 `validated-callback / verified:true / run`；空函数、错误 owner 和未经验证的候选均不得伪装成可执行 cleanup。V7.4.17 当前为 `65 legacy-callback + 21 validated-callback + 2 missing`。
