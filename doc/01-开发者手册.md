...
| 服务 | 职责 |
| ---- | ---- |
| `app.services.access_control` | 学校、班级、课程可见性与角色范围判断；普通业务端点应复用它，避免 `_require_*` 逻辑漂移；教师端班级挂课、班级成员列表、学生成员状态维护、学习事件、积分流水、学生进度、作业提交列表、批改与待批改队列必须落到 active class teacher scope；学校/班级深度统计分别要求 school teacher scope / class teacher scope，非授权用户不通过统计端点暴露对象存在性；课程结构写入要求课程作者或全局 admin；全局 admin 保留跨范围治理能力 |
...
- 班级成员列表与状态维护先作为教师端学生管理底座：`GET /api/classes/{id}/members` 只允许全局 admin 或该班 active teacher membership 读取，默认仅返回 active 成员，响应包含成员关系状态和用户状态，不暴露密码、会话或联系方式；`PATCH /api/classes/{id}/members/{membership_id}` 只允许维护 student membership 的 `active/inactive`，教师 membership、移除、转班、批量导入和监护人/联系方式等复杂学生管理另行设计。
