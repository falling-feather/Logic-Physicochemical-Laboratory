...
- cleanup 状态必须使用 `legacy-callback`、`candidate-unwired` 或 `missing` 并保持 `verified:false`，直到对应任务完成真实离开/重入验证；候选项禁止配置空函数或未经验证的可执行 `run`。
...
5. **实现并验证生命周期后升级 cleanup 状态**：先提供真实 destroy/取消协议，再通过独立离开/重入证据将 `missing` 或 `candidate-unwired` 升级；Router 不再登记实验 destroy。
