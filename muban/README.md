...
本文件夹提供星序页面的 **布局、字号、控件调度、Canvas/DOM 分工** 参考模板。
后续新建页面时，请先判断页面层级，再选择对应模板；不要把首页、子站 Hero、学科画廊、单实验页面混用。

## 📦 本目录所有模板

| 层级 | 模板 | 适用场景 |
|---|---|---|
| L0 顶层入口 | [02-多星系顶层导航.md](02-多星系顶层导航.md) | 星序多星系选择 + 子层学科行星导航大屏 |
| L1 星系首页 | [01-工科实验室主页布局.md](01-工科实验室主页布局.md) | 工科实验室/未来站内星系的星图式首页（品牌核心 + 学科节点） |
| L1 独立子站 | [03-子站首页布局.md](03-子站首页布局.md) | 代码空间等独立子站 Hero + 功能区首页 |
| L2 学科目录 | [04-学科画廊布局.md](04-学科画廊布局.md) | 学科页 Hero + 模块卡片画廊 + 实验视图切换 |
| L3 单实验 | [template.html](template.html) | 单个实验页面（控件 + Canvas + 实时数据 + 教育面板） |

## 统一布局原则（v6.2）

1. **入口层级必须分清**：L0 `#planets` 主标题写「星序 / Astra」，承担多星系总入口；L1 `#home` 主标题写当前星系名（如「工科实验室」），副标说明实验数量或星系定位。不要把 L0 顶层品牌和 L1 星系首页混用。
2. **Canvas 负责画面，DOM 负责文字和控件**：大段说明、导航、按钮、标签、表单都应保持 DOM 原生，Canvas 只承担可视化主体。
3. **首页少用面板**：星系首页只保留品牌核心、学科节点、少量 HUD 辅助信息；不要放大卡片网格或大段说明。
4. **卡片只用于重复内容**：实验卡片、特性卡片可以用卡片；页面 section 不要再包一层“漂浮大卡片”。
5. **字体节奏统一**：页面标题使用 `--font-display`，正文/控件使用 `--font-sans`，数据/公式使用 `--font-mono`；不要在 Canvas 中直接写 CSS `var()` 字体。
6. **移动端先压缩装饰，再压缩内容**：先隐藏 HUD 数据、星云、长描述，再缩小轨道/节点；不要让文字覆盖可点击目标。
