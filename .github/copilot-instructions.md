...
- 🌌 **工科实验室星系**（主站，本仓库根目录）：数学、物理、化学、算法、生物五大学科，共 **65 个可视化实验**，对齐人教版 2019 新课标
...
- **渲染**：可视化主要使用 Canvas 2D API + requestAnimationFrame + performance.now() dt 驱动，少量实验采用 DOM/CSS 可视化
...
| 文档/文件 | 内容 | 何时阅读 |
|-----------|------|----------|
| `doc/01-开发者手册.md` | **完整开发者文档**（v6.0 双子站架构）：架构、目录结构、65 个实验清单、JS 加载顺序、路由/模块选择器/首页系统、加载屏优化、开发指南、CSS 设计系统、部署、更新日志 | 首次接触项目时通读 |
...
| `README.md` | 项目简介 + 快速开始 + 65 个实验一览表 | 快速概览 |

## 关键文件速查

```
index.html                      → 唯一 HTML 入口，所有页面 <section> + 全部 <script> 引用
shared/js/config.js             → 全局配置（5 学科元数据 + 65 个实验条目 + 多星系拓扑）
shared/js/router.js             → hash 路由 + GSAP 页面转场 + 页面生命周期
...
1. **实验模块模式**：每个实验封装为全局对象（如 `Calculus`、`RedoxReaction`），暴露 `initXxx()` 全局函数，由 `ModuleSelector` 在打开卡片时懒加载，离页时由 `router.js` 统一清理
2. **Canvas 标准**：DPR 适配（`canvas.width = rect.width * dpr`）+ `ResizeObserver` 监听容器 + `ctx.setTransform(dpr, 0, 0, dpr, 0, 0)`
3. **脚本加载**：CDN 库 + 框架脚本（同步）→ home.js / planets.js / main.js（同步）→ 65 个实验模块（全部 `defer`）
...
- **数学** (`pages/mathematics/`)：15 个实验 — 函数、微积分、几何、复数、三角、集合、概率、向量、不等式、圆锥曲线、立体几何、排列组合、数列、函数性质、指数与对数
- **物理** (`pages/physics/`)：17 个实验 — 力学、运动学、抛体、圆周、万有引力、能量、电磁、电路、电磁感应、交变电流、波动、光学、流体、相对论、力的合成与分解、动量守恒、带电粒子运动
- **化学** (`pages/chemistry/`)：12 个实验 — 周期表、分子结构、化学反应、化学键、离子反应、氧化还原、化学平衡、电化学、有机化学、反应速率、溶液电离、原子结构与电子排布
