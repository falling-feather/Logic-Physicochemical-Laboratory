...
> **最新治理**: 2026-07-07，`houduan` 已补齐 BE-12 审计链完整性校验：`GET /api/admin/audit-logs/chain-integrity` 可按时间窗和 `limit/issue_limit` 扫描审计日志，复用 `app.services.audit_chain` 重算 `current_hash`、检查相邻 `prev_hash` 链接、把历史空 hash 标记为 partial，并写入不含 issue 明细的 `admin.audit.chain_integrity` 摘要；该接口只读，不修复、不删除、不提供 WORM、第三方时间戳或外部锚定。
