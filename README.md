...
> **当前状态**: v6.5（2026-07-03 起）— `houduan` 分支已接入 Python 后端骨架、健康检查、部署预检、部署 smoke 门禁、API no-store 缓存边界、工科试验室内容协议持久化样例、内容 seed 启动初始化与读取无副作用边界、ContentDraft 草稿与脚本审核、草稿提交/退回/撤回工作流、内容发布/版本记录/回滚、发布元数据回填、管理端版本 diff、本地账号认证安全基线、学校/班级最小闭环与加入申请审批、课程/作业/学习事件/提交批改/积分统计、知识状态/班级规则统计、个人/班级知识快照、知识快照周期重算脚本、运行记录与进程内调度器、管理端 API、学校/班级深度统计、管理端加入申请队列、管理端列表分页搜索、待批改队列、审计元数据与认证事件审计、学校/班级/课程访问控制服务层、跨范围权限矩阵测试，以及前端 opt-in schema 渲染试点
...
- **CMake 3.14+** — C++ 构建系统
- **Python 3 / FastAPI / SQLAlchemy** — v6.5 业务后端骨架、API、MySQL 连接、部署预检、部署 smoke、API no-store 缓存边界、内容协议、内容 seed 初始化与只读查询边界、ContentDraft 草稿与脚本审核、内容发布/版本记录/追加式回滚、本地认证安全基线、学校/班级加入申请审批、学校/班级/课程访问控制服务层、作业提交/批改、知识状态/班级规则统计、个人/班级知识快照、周期重算运行记录与进程内调度器、管理端学校/班级统计、加入申请治理、列表分页搜索、待批改队列和审计元数据
