...
- 当前稳定发布基线为 V7.5.10，QA-010 已对精确提交 `9b0ea63cc7257a8eaaaca0f615de6af35248e17f` 使用真实外部 Edge 150 完成最终复验并 PASS，P0/P1/P2/P3 均为 0。当前 `review` 候选已在该基线上完成 `re1`—`re14`：代码挑战焦点与提交终态、未来课程初始化/互动证据、课程发布并发写门禁、多班级作业历史、高基数分页、判题租约恢复、教师分页消费、兼容列表上限和教师样式分层均已按独立切片关闭；Alembic 当前 head 为 `20260719_0050`，默认 `DisabledCodeRunnerAdapter` 仍只诚实持久化 `runner_unavailable`。教师样式按 foundation/workbench/curriculum 顺序按角色加载，主壳与角色资源代际为 `20260719v75ReviewTeacherLayersP0`。QA-011 尚未执行，因此 review 只称“候选”，不替代 V7.5 的 QA-010 结论。旧 V6.6 `review` 引用保留为 `legacy/review-v6.6`，不改写历史；真实 MySQL、隔离 runner 与 staging 仍未补证，本机结果不得外推。唯一任务状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
