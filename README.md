...
> **最新治理**: 2026-07-07，`houduan` 已补齐 BE-12 缺陷记录外部 issue 链接字段：`BugRecord` 可保存 `external_issue_provider`、`external_issue_id` 和 `external_issue_url`，管理端创建/更新/搜索和审计快照均已覆盖；正式外部 issue 自动创建、双向同步、归档、防篡改和高频事件策略仍属后续治理。
