...
- 当前集成基线为 V7.5.3，前端稳定表面仍以 V7.4.37 为基线。V7.5.3 已交付三星系共用的稳定课程/活动键、班级分块 `hidden/locked/open`、计划开放、前置完成、乐观并发和教师学生进度矩阵；学生课程、事件、作业与提交统一执行服务端可见性，Alembic head 为 0048。代码空间和未来星系重绘仍由隔离工作组交付，代码提交/OJ 与教师前端按后续版本推进，QA 继续独立验收；V7.5 规划与实际完成状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
