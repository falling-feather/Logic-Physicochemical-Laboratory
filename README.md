...
- 当前集成基线为 V7.5.4。V7.5.3 已交付三星系共用的稳定课程/活动键、班级分块 `hidden/locked/open`、计划开放、前置完成、乐观并发和教师学生进度矩阵，Alembic head 为 0048；V7.5.4 又把代码空间重绘为 6 组课程目录、独立子课程和 18 个四语言互动活动，浏览器预检明确不冒充正式判题，页脚与工科试验室采用同一信息结构。本地运行依赖固定版本、许可证与 SHA-256 清单，C/C++ 只在可终止 Worker 内执行。未来星系、代码提交/OJ 与教师前端按后续版本推进，QA 继续独立验收；V7.5 规划与实际完成状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
