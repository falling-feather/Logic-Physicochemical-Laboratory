...
- 当前集成基线为 V7.5.10。V7.5.3—V7.5.9 已交付三星系稳定课程/活动键、班级 `hidden/locked/open` 发布计划、代码空间和未来星系各 6 组/18 活动、provider-neutral 代码提交/判题契约、Cookie 会话学生上下文、星序教师课程节奏/进度/代码审阅，以及资源、性能、缓存、9001、44px 触达和键盘导航收束。QA-010 对精确提交 `9b0ea63cc7257a8eaaaca0f615de6af35248e17f` 使用真实外部 Edge 150 完成最终复验：Future 核心 hit/focus 区、教师 tablist 左右/上下/Home/End/循环行为、根宽与控制台全部通过，首轮角色权限、课程链、OJ `runner_unavailable` 诚实降级、性能、9001 和只读账本证据继续有效；最终 PASS，P0/P1/P2/P3 均为 0。V7.5 已满足进入 `review` 的门禁；真实 MySQL、隔离 runner 与 staging 仍未补证，本机结果不得外推。唯一任务状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
