...
- 当前集成基线为 V7.5.8。V7.5.3—V7.5.7 已交付三星系稳定课程/活动键、班级分块 `hidden/locked/open`、计划开放与乐观并发、代码空间和未来星系各 6 组/18 活动的课程目录—子课路径、provider-neutral 代码提交/判题契约、Cookie 会话学生上下文，以及星序教师工作台中的跨三星系课程节奏、学生进度与代码审阅。V7.5.8 完成综合收束：未来星系首屏背景由 3,762,137 字节 PNG 压缩为 136,676 字节 WebP（减少约 96.4%）并退出全局预缓存；互动运行时改为路由按需加载，轨道光源使用椭圆焦点，Canvas 限制 DPR 并响应减少动效；代码空间静态代际统一为 `758r1`；教师进度与代码提交改为服务端分页。前端 193 个受跟踪脚本和 31/31 合同、后端 489 项通过/7 项真实 MySQL 条件跳过、依赖锁与审计、C++ 构建清单以及 9001 启动/重复启动/停止/重启、私有路径 404 和 WebP MIME 均已通过主开发门禁；独立 `QA-010/V7.5.9` 仍是进入 `review` 前的下一步。默认 runner 关闭，提交诚实记录为 `runner_unavailable`；本机结果不外推为 staging/production。唯一任务状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
