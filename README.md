...
> **最新治理**: 2026-07-07，`houduan` 已补齐 BE-02 内容协议稳定身份：`sectionId/sourceId` 已进入 schema，历史读取保持可选兼容，新建/编辑/发布草稿和内置内容初始化要求稳定身份，semantic diff 优先按稳定 ID 识别章节与来源；上一片内容脚本资产 allowlist/SRI 静态门禁继续保留。
> **最新回归**: 2026-07-07，内容协议稳定身份已覆盖重复 `sectionId/sourceId`、非法格式、草稿缺失/冲突身份、发布前稳定身份复核和 semantic diff 按稳定 ID 把标题/来源变更识别为修改；真实 iframe/worker 执行隔离、浏览器 CSP 强制、后端下载校验、供应链治理、管理端 diff 响应显式暴露 section/source ID 字段和更细脱敏仍属后续。
