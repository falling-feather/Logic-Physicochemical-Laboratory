...
> **当前状态**: v6.5（2026-07-03 起）— `houduan` 分支已接入 Python 后端骨架、健康检查、工科试验室内容协议持久化样例、本地账号与学校/班级最小闭环、课程/作业/学习事件/提交批改/积分统计、知识状态/班级规则统计、管理端 API 与审计日志基础，以及前端 opt-in schema 渲染试点
...
- **CMake 3.14+** — C++ 构建系统
- **Python 3 / FastAPI / SQLAlchemy** — v6.5 业务后端骨架、API、MySQL 连接、内容协议、作业提交/批改、知识状态/班级规则统计、管理端 API 和审计日志基础
