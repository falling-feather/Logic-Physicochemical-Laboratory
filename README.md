...
- 当前集成基线为 V7.5.5。V7.5.3 已交付三星系共用的稳定课程/活动键、班级分块 `hidden/locked/open`、计划开放、前置完成、乐观并发和教师学生进度矩阵，Alembic head 为 0048；V7.5.4 把代码空间重绘为 6 组课程目录、独立子课程和 18 个四语言互动活动，浏览器预检明确不冒充正式判题。V7.5.5 又把未来星系从散乱长页收束为 6 组/18 活动的课程目录—子课路径，6 个按需 Canvas owner 服务具体观察任务，代表课“轨道与尺度”仅在需要时加载本地锁定的 `three@0.185.1`，并为无 WebGL、低性能和减少动效提供可完成的 Canvas 降级；两个星系页脚均沿用工科试验室的信息列与版权行结构。代码提交/OJ 与教师前端按后续版本推进，QA 继续独立验收；V7.5 规划与实际完成状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
