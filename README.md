...
> **最新治理**: 2026-07-07，`houduan` 已补齐 BE-12 审计留存预检：`GET /api/admin/audit-logs/retention-plan` 基于 `ASTRA_AUDIT_LOG_RETENTION_DAYS` 或显式 `retention_days/before` 计算归档候选、临期数量、聚合分布和哈希链边界，并记录 `admin.audit.retention_plan` 摘要；该接口只做 observe-only 预检，不删除、不真实归档、不做 WORM 或外部锚定。
