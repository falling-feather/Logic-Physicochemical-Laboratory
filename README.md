...
- 当前集成候选为 V7.5.9。V7.5.3—V7.5.8 已交付三星系稳定课程/活动键、班级 `hidden/locked/open` 发布计划、代码空间和未来星系各 6 组/18 活动、provider-neutral 代码提交/判题契约、Cookie 会话学生上下文、星序教师课程节奏/进度/代码审阅，以及资源、性能、缓存和 9001 综合收束。QA-010 在真实 Edge 对 V7.5.8 的首轮独立验收中，仅因未来星系核心交互不足 44px 与教师 tablist 缺少方向键 roving 行为以 P2 退回，其余角色权限、课程链、OJ `runner_unavailable` 诚实降级、性能和同源矩阵均通过。V7.5.9 已修复两项 P2 和相邻移动触达面，主壳/教师/未来代际为 `20260719v759A11yP0`、代码空间为 `759r1`，193 个受跟踪 JavaScript 与 31/31 前端合同通过；当前等待同一 QA 组精确复验，PASS 前不创建 `review`。真实 MySQL、隔离 runner 与 staging 仍未补证，本机结果不得外推。唯一任务状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
