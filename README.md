...
> **当前状态**: v6.5（2026-07-03 起）— `houduan` 分支已接入 Python 后端骨架、健康检查、部署预检、API no-store 缓存边界、工科试验室内容协议持久化样例、本地账号认证安全基线、学校/班级最小闭环、课程/作业/学习事件/提交批改/积分统计、知识状态/班级规则统计、管理端 API、学校/班级深度统计、管理端列表分页搜索、待批改队列、审计元数据与认证事件审计，以及前端 opt-in schema 渲染试点
...
- **CMake 3.14+** — C++ 构建系统
- **Python 3 / FastAPI / SQLAlchemy** — v6.5 业务后端骨架、API、MySQL 连接、部署预检、API no-store 缓存边界、内容协议、本地认证安全基线、作业提交/批改、知识状态/班级规则统计、管理端学校/班级统计、列表分页搜索、待批改队列和审计元数据
