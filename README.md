...
- 当前稳定发布基线为 V7.5.10；增量 `review` 已完成 `re1`—`re15` 并由 QA-011 对精确候选 `e8768949f7787fa7cba43d0e92c7b8804c12f631` 使用真实外部 Edge 150 独立终验 PASS，P0/P1/P2/P3 均为 0。审查已关闭离线运行依赖、代码挑战焦点/提交终态、未来课程初始化/互动证据、课程发布并发写门禁、多班级历史、高基数分页、判题租约恢复、兼容列表上限和教师样式分层；Alembic 当前 head 为 `20260719_0050`，默认 `DisabledCodeRunnerAdapter` 仍只诚实持久化 `runner_unavailable`。独立门禁为 197 个受跟踪 JavaScript、35/35 前端合同、后端 519 passed/7 个真实 MySQL 条件跳过；9001、三角色、三星系、桌面/390×844 与只读账本均通过。`re16` 仅记录终验和团队收束，不改变受验运行时代码；真实 MySQL、隔离 runner、staging 与公网 R6 仍未补证，本机结果不得外推。唯一任务状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
