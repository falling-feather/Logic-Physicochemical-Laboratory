...
- 当前集成基线为 V7.5.6。V7.5.3 已交付三星系共用的稳定课程/活动键、班级分块 `hidden/locked/open`、计划开放、前置完成、乐观并发和教师学生进度矩阵；V7.5.4—V7.5.5 又分别把代码空间与未来星系重绘为 6 组/18 活动的课程目录—子课路径，并补齐可终止浏览器运行时、六方向 Canvas、按需 Three.js、降级与统一页脚。V7.5.6 将 Alembic head 推进到 0049，新增不可变题目版本、学生源码提交、判题尝试/租约、教师分页查看、稳定活动查询与服务端发布状态裁剪；当前默认 runner 关闭，提交会诚实记录为 `runner_unavailable`，API 进程没有执行、shell、动态求值或外呼学生源码的路径。正式提交 UI 和三星系教师编排由 FE-010 接入，QA 继续独立验收；V7.5 规划与实际完成状态以 [`doc/02-项目规划.md`](doc/02-项目规划.md) 为准。
