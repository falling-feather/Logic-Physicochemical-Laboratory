...
> **最新治理**: 2026-07-07，`houduan` 已补齐 BE-02 内容 diff 敏感预览：管理端版本 diff 的 raw `changes` 与 semantic `field_changes/prop_changes` 会对 `token/key/secret/script/sandbox/integrity/crossorigin` 等敏感字段返回结构化 redaction preview，非敏感字段继续保持原值；上一片 semantic diff 稳定身份响应、稳定 `sectionId/sourceId` 写入契约和内容脚本资产 allowlist/SRI 静态门禁继续保留。
> **最新回归**: 2026-07-07，内容 diff 敏感预览已覆盖 raw path diff、semantic props、嵌套敏感字段和 `description` 等非敏感字段不误伤；真实 iframe/worker 执行隔离、浏览器 CSP 强制、后端下载校验和供应链治理仍属后续。
