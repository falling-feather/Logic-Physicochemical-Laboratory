...
- V7.4.13 建立 19 个主站页面的只读注册表并通过 V7.4.15 独立 QA；V7.4.16 的 88 项冻结实验注册结构已通过 QA-002，V7.4.17 又将其中 21 项升级为经离开/重入验证的 cleanup。当前边界为 65 个旧兼容回调、21 个已验证回调和 2 个缺失取消协议；可执行清理仅用于整页离开，实验切换/返回画廊仍等待统一收束。
